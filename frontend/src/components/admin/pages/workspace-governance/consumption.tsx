...
const SAMPLE_CONSUMPTION: Consumption[] = CONS_SEED.map(
  ({ resource, type, unit }, i) => {
    ...
    const growth = -8 + (n % 32);
    const status: Status =
      growth >= 20 ? "Spiking" : growth >= 8 ? "Rising" : "Healthy";
    return {
      id,
      resource,
      resType: type,
...
export function ConsumptionView() {
  ...
  const avgUtil = Math.round(
    (records.reduce((a, r) => a + r.average, 0) / current) * 100,
  );
  ...
  const topConsumer =
    [...records].sort((a, b) => b.current - a.current)[0]?.workspace ?? "—";
  ...
  const cols: Column<Consumption>[] = [
    ...
    },
    {
      ...
      render: (r) => `${r.average} ${r.unit}`,
    },
    {
      key: "peak",
      header: "Peak Usage",
      sortValue: (r) => r.peak,
      render: (r) => `${r.peak} ${r.unit}`,
    },
    {
      key: "growth",
      header: "Growth",
      sortValue: (r) => r.growth,
      render: (r) => <GrowthBadge growth={r.growth} />,
    },
    {
      key: "status",
      header: "Status",
  ...
  return (
    <>
      ...
      <DiscoveryListView
        ...
        presets={[{ label: "All consumption", onApply: clearFilters }]}
        filterRightSlot={
        ...
        bulkActions={(ids, clear) => (
          <>
            ...
            <HeaderButton icon={<TrendingUp size={13} />} onClick={clear}>
              Forecast
            </HeaderButton>
          ...
          <KVGrid
            ...
            items={[
              ...
              { k: "Peak Usage", v: `${rec.peak} ${rec.unit}`, sample: true },
              {
                ...
                v: `${rec.available} ${rec.unit}`,
                sample: true,
              },
              {
                k: "Forecast",
                v: `${Math.round(rec.current * (1 + rec.growth / 100))} ${rec.unit}`,
                sample: true,
              },
              {
                k: "Monthly Growth",
    ...
    <Section title="Consumption cost" sample>
      <KVGrid
        items={[
          ...
          {
            k: "Projected Cost",
            v: `$${Math.round(rec.monthlyCost * (1 + rec.growth / 100))}k`,
            sample: true,
          },
  ...
  return (
    <>
      ...
      <Section title="Consumption forecast" sample>
        <StatRow
          label="Growth Trend"
          value={`${rec.growth >= 0 ? "+" : ""}${rec.growth}% / month`}
          sample
        />
        <StatRow
          ...
          value={
            ran
              ? `${Math.round(rec.current * (1 + (rec.growth * 3) / 100))} ${rec.unit} in 90d`
              : "Run forecast"
          }
          sample
        />
        <StatRow
          ...
          tone={rec.status === "Spiking" ? "warn" : "ok"}
          sample
        />
        <StatRow
          label="Recommended Capacity"
          value={ran ? `${Math.round(rec.current * 1.4)} ${rec.unit}` : "—"}
          sample
        />
