...
type PolicyType = "Allocation" | "Scaling" | "Approval" | "Exception" | "Enforcement";
type Scope = "Organization" | "Business Unit" | "Workspace" | "Environment" | "Cloud Provider" | "Workspace Type";
type Priority = "Critical" | "High" | "Medium" | "Low";
type Enforcement = "enforced" | "monitoring" | "draft" | "scheduled" | "partially-enforced";
type Status = "Active" | "Draft" | "Disabled";

const SCOPES: Scope[] = ["Organization", "Business Unit", "Workspace", "Environment", "Cloud Provider", "Workspace Type"];
const WORKSPACES = ["Global", "Payments", "Data Lake", "Retail Web", "Analytics"];
...
const OWNERS = ["Capacity Admin", "Governance Admin", "Platform Team", "FinOps"];
...
export function QuotaPoliciesView() {
  ...
  const hasFilters = !!(search || fType || fWs || fBu || fEnv || fStatus || fPriority);
  const clearFilters = () => {
    setSearch("");
    setFType("");
    setFWs("");
  ...
  const protectedWorkspaces = records.reduce((a, r) => a + r.assignedWorkspaces, 0);
  ...
  const approvalRequests = records.filter((r) => r.type === "Approval").reduce((a, r) => a + r.quotaAssignments, 0);
  const coverage = Math.round((active / records.length) * 100);
  const policyHealth = 100 - Math.min(30, violations * 2);

  const toolbar: CommandItem[] = [
    { key: "create", label: "Create Policy", icon: <Plus size={15} />, onClick: () => navigate("/admin/workspace-governance?tab=capacity") },
    { key: "edit", label: "Edit Policy", icon: <Pencil size={15} />, disabled: true },
    { key: "clone", label: "Clone Policy", icon: <Copy size={15} />, disabled: true },
    { key: "disable", label: "Disable Policy", icon: <Power size={15} />, disabled: true },
    { key: "delete", label: "Delete Policy", icon: <Trash2 size={15} />, disabled: true },
  ...
  return (
    <>
      <PostureGrid>
        <PostureCard title="Active Policies" value={active} tone="ok" sub={<>Governing quotas <SampleTag /></>} />
        <PostureCard title="Protected Workspaces" value={protectedWorkspaces} tone="ok" sub={<>Under quota policy <SampleTag /></>} />
        <PostureCard title="Quota Violations" value={violations} tone={violations > 0 ? "warn" : "ok"} sub={<>Policy breaches <SampleTag /></>} />
        <PostureCard title="Policy Exceptions" value={exceptions} tone={exceptions > 0 ? "warn" : "ok"} sub={<>Approved deviations <SampleTag /></>} />
        <PostureCard title="Auto Scaling Policies" value={autoScaling} tone="ok" sub={<>Elastic quotas <SampleTag /></>} />
        <PostureCard title="Approval Requests" value={approvalRequests} tone="ok" sub={<>Awaiting decision <SampleTag /></>} />
        <PostureCard title="Compliance Coverage" value={`${coverage}%`} tone={coverage >= 80 ? "ok" : "warn"} sub={<>Workspaces governed <SampleTag /></>} />
        <PostureCard title="Policy Health" value={`${policyHealth}%`} tone={policyHealth >= 85 ? "ok" : "warn"} sub={<>Enforcement posture <SampleTag /></>} />
      </PostureGrid>
      ...
      >
        ...
        <DirectoryTable
          ...
          bulkActions={(ids, clear) => (
            <>
              <HeaderButton icon={<Power size={13} />} onClick={clear}>Enable ({ids.length})</HeaderButton>
...
const DRAWER_TABS = [
  ...
  { id: "allocation", label: "Allocation", icon: <SlidersHorizontal size={13} /> },
      ...
      {sub === "statistics" && (
        <Section title="Statistics" sample>
          <KVGrid
            cols={3}
            items={[
              { k: "Assigned Workspaces", v: rec.assignedWorkspaces, sample: true },
...
function ScopeTab({ rec }: { rec: Policy }) {
  return (
    <Section title="Scope — where the quota policy applies" sample>
      <div style={{ fontSize: 12.5, color: T.textMuted, marginBottom: 10 }}>Organization → Business Unit → Workspace → Quota Policy.</div>
      {SCOPES.map((s) => (
...
function RulesTab() {
  const rules = ["Default Quota", "Maximum Quota", "Minimum Quota", "Quota Increase Limits", "Quota Decrease Limits", "Auto Scaling Allowed", "Approval Required", "Temporary Quotas", "Expiration Required", "Reserved Capacity Required", "Business Hours Restrictions"];
  const operators = ["Equals", "Not Equals", "Greater Than", "Less Than", "Contains", "In"];
  const actions = ["Allow", "Deny", "Require Approval", "Auto Increase", "Auto Decrease", "Notify", "Block"];
  return (
    <Section title="Rules — quota governance behavior (Condition · Operator · Value · Action)" sample>
      <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        <Select label="Condition" value="Maximum Quota" onChange={() => {}} options={rules.map((r) => ({ value: r, label: r }))} />
        <Select label="Operator" value="Greater Than" onChange={() => {}} options={operators.map((o) => ({ value: o, label: o }))} />
        <Select label="Value" value="500 units" onChange={() => {}} options={[{ value: "500 units", label: "500 units" }]} />
        <Select label="Action" value="Require Approval" onChange={() => {}} options={actions.map((a) => ({ value: a, label: a }))} />
...
function AllocationTab() {
  const strategies = ["Fixed Allocation", "Dynamic Allocation", "Reserved Allocation", "Shared Capacity", "Department Allocation", "Environment Allocation"];
  return (
    <Section title="Allocation — quota allocation behavior" sample>
      <StatRow label="Allocation Strategy" value="Dynamic Allocation" tone="ok" sample />
      ...
      <Section title="Supported strategies">
        {strategies.map((s, i) => (
          <StatRow key={s} label={s} value={i === 1 ? "Selected" : "Available"} tone={i === 1 ? "ok" : undefined} sample />
        ))}
      </Section>
    </Section>
  );
...
function ScalingTab() {
  const modes = ["Disabled", "Manual", "Scheduled", "Automatic", "Demand-Based", "Predictive"];
  return (
    <Section title="Scaling — automatic quota scaling" sample>
      ...
      <Section title="Supported modes">
        {modes.map((m, i) => (
          <StatRow key={m} label={m} value={i === 3 ? "Selected" : "Available"} tone={i === 3 ? "ok" : undefined} sample />
        ))}
      </Section>
    </Section>
  );
...
function ExceptionsTab({ rec }: { rec: Policy }) {
  const list = Array.from({ length: rec.exceptions }, (_, i) => {
    ...
    return {
      id: `${rec.id}-e-${i}`,
      exception: pick(["Temporary Increase", "Emergency Capacity", "Executive Override", "Disaster Recovery", "Migration Window", "Testing Exception"], m),
  ...
  return (
    <>
      ...
      {list.length ? <DirectoryTable columns={cols} rows={list} /> : <EmptyState icon={<FilePlus2 size={18} />} title="No exceptions" hint="No temporary quota exceptions under this policy." />}
    </>
  );
}
  ...
  return (
    <Section title="Simulation — test policy behavior before deployment" sample>
      <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        ...
        <Select label="Current Quota" value="200 units" onChange={() => {}} options={[{ value: "200 units", label: "200 units" }]} />
        <Select label="Requested Quota" value="320 units" onChange={() => {}} options={[{ value: "320 units", label: "320 units" }]} />
        <Select label="Growth Rate" value="+12%/mo" onChange={() => {}} options={[{ value: "+12%/mo", label: "+12%/mo" }]} />
      ...
      {ran ? (
        <>
          ...
          <StatRow label="Reason" value="Increase exceeds +25% auto-scale threshold" sample />
          <StatRow label="Recommendation" value="Route to Finance Approval" sample />
        </>
      ) : (
...
function AuditTab() {
  const events = ["Policy Created", "Policy Modified", "Policy Assigned", "Policy Removed", "Simulation Executed", "Exception Approved", "Policy Disabled"];
  return (
    <>
