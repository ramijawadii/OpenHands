...
type Status = "Pending Approval" | "Approved" | "Rejected" | "Expired" | "Cancelled";

const REQUEST_TYPES: RequestType[] = ["Resource Access", "Secrets Access", "API Access", "Database Access", "Network Connectivity", "Identity Federation", "AI Service Access", "Data Sharing", "Workspace Peering", "Emergency Access", "Policy Exception"];
...
const REQUESTERS = ["Aisha Khan", "David Chen", "Marco Rossi", "Priya Nair", "Sara Lopez"];
const APPROVERS = ["Workspace Owner", "Security Team", "Governance Admin", "Compliance Office"];
const FRAMEWORKS = ["ISO 27001", "SOC 2", "NIST", "PCI DSS", "HIPAA", "CSA CCM"];

const PRIORITY_TONE: Record<Priority, string> = { Critical: T.danger, High: T.warning, Medium: T.accent, Low: T.textMuted };
const PRIORITY_ORDER: Record<Priority, number> = { Critical: 0, High: 1, Medium: 2, Low: 3 };
const STATUS_TONE: Record<Status, string> = { "Pending Approval": T.warning, Approved: T.success, Rejected: T.danger, Expired: T.textMuted, Cancelled: T.textMuted };
...
const SAMPLE_REQUESTS: Request[] = Array.from({ length: 15 }, (_, i) => {
  ...
  const status = pick<Status>(["Pending Approval", "Pending Approval", "Approved", "Approved", "Rejected", "Expired", "Cancelled"], n);
  return {
    id,
...
export function CrossWorkspaceRequestsView() {
  ...
  const rows = records.filter((r) => {
    ...
    return (
      (!q || r.id.toLowerCase().includes(q) || r.title.toLowerCase().includes(q) || r.requester.toLowerCase().includes(q)) &&
      (!fType || r.type === fType) &&
  ...
  const hasFilters = !!(search || fType || fSource || fTarget || fStatus || fPriority || fBu);
  const clearFilters = () => {
    setSearch("");
    setFType("");
    setFSource("");
  ...
  const incoming = records.filter((r) => r.status === "Pending Approval").length;
  ...
  const toolbar: CommandItem[] = [
    { key: "create", label: "Create Request", icon: <Plus size={15} />, onClick: () => navigate("/admin/workspace-governance?tab=cross") },
    { key: "approve", label: "Approve", icon: <Check size={15} />, disabled: true },
    { key: "reject", label: "Reject", icon: <X size={15} />, disabled: true },
    { key: "cancel", label: "Cancel", icon: <Ban size={15} />, disabled: true },
    { key: "withdraw", label: "Withdraw", icon: <Undo2 size={15} />, disabled: true },
    ...
    { key: "deps", label: "Check Dependencies", icon: <Network size={15} />, disabled: true },
    ...
    { key: "bulk", label: "Bulk Approve", icon: <CheckCheck size={15} />, disabled: true },
  ];
  ...
  const cols: Column<Request>[] = [
    ...
    { key: "source", header: "Source Workspace", sortValue: (r) => r.source, render: (r) => r.source },
    { key: "target", header: "Target Workspace", sortValue: (r) => r.target, render: (r) => r.target },
    { key: "priority", header: "Priority", sortValue: (r) => PRIORITY_ORDER[r.priority], render: (r) => <PriorityBadge priority={r.priority} /> },
    { key: "submitted", header: "Submitted", sortValue: (r) => r.created, render: (r) => r.submitted },
  ...
  return (
    <>
      <PostureGrid>
        ...
        <PostureCard title="Incoming Requests" value={incoming} tone="ok" sub={<>Received from others <SampleTag /></>} />
        <PostureCard title="Outgoing Requests" value={outgoing} tone="ok" sub={<>Submitted by us <SampleTag /></>} />
        <PostureCard title="Approved Requests" value={approved} tone="ok" sub={<>Completed successfully <SampleTag /></>} />
        <PostureCard title="Rejected Requests" value={rejected} tone={rejected > 0 ? "warn" : "ok"} sub={<>Denied by reviewers <SampleTag /></>} />
        <PostureCard title="Expired Requests" value={expired} tone="ok" sub={<>Missed deadlines <SampleTag /></>} />
        <PostureCard title="Avg Approval Time" value={`${avgApproval}h`} tone={avgApproval <= 24 ? "ok" : "warn"} sub={<>Submit to decision <SampleTag /></>} />
        <PostureCard title="Policy Violations" value={violations} tone={violations > 0 ? "warn" : "ok"} sub={<>Flagged in review <SampleTag /></>} />
      </PostureGrid>
      ...
      >
        ...
        >
          <Select label="Request Type" value={fType} onChange={setFType} options={facet(records.map((r) => r.type))} />
          ...
          <Select label="Priority" value={fPriority} onChange={setFPriority} options={facet(records.map((r) => r.priority))} />
...
function RequestDrawer({ rec, onClose }: { rec: Request; onClose: () => void }) {
      ...
      {sub === "general" && (
        <Section title="General">
          ...
          <div style={{ fontSize: 12.5, color: T.textNav, paddingTop: 6 }}>
            {rec.source} requests {rec.type.toLowerCase()} from {rec.target}, subject to approval and compliance validation.
          </div>
        </Section>
      )}
      {sub === "statistics" && (
        <Section title="Statistics" sample>
          <KVGrid
            ...
            items={[
              ...
              { k: "Estimated Completion", v: rec.status === "Approved" ? "Complete" : "2 days", sample: true },
            ]}
          />
        </Section>
      )}
    ...
    <Section title="Request details" sample>
      <KVGrid
        items={[
          ...
          { k: "Business Justification", v: `Operational need for ${rec.type.toLowerCase()}`, sample: true },
          { k: "Requested Duration", v: pick(["30 days", "90 days", "Permanent"], hashId(rec.id)), sample: true },
          ...
          { k: "Approval Requirement", v: `${rec.approvals}-stage workflow`, sample: true },
        ]}
      />
    </Section>
  );
...
function ResourcesTab({ rec }: { rec: Request }) {
  const list = Array.from({ length: rec.resources }, (_, i) => {
    ...
    return {
      id: `${rec.id}-r-${i}`,
      resource: pick(["Central Secrets", "Shared DB", "AI Gateway", "API Gateway", "Shared VPC"], m),
  ...
  return (
    <>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
        ...
        <HeaderButton icon={<ClipboardCheck size={13} />}>Validate</HeaderButton>
        <SampleTag />
      </div>
...
function ApprovalTab({ rec }: { rec: Request }) {
  const stages = ["Request", "Workspace Owner", "Security Review", "Governance Review", "Compliance Review", "Final Approval"];
  const completed = rec.status === "Approved" ? stages.length : rec.status === "Rejected" ? 2 : rec.approvals;
  return (
    <>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
        <HeaderButton icon={<Check size={13} />}>Approve</HeaderButton>
            ...
            <div key={s} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 11.5, color: T.textMuted }}>
                  {rejected ? `${pick(APPROVERS, hashId(rec.id))} · rejected` : done ? `${pick(APPROVERS, hashId(rec.id) + i)} · approved` : "Pending"}
                </div>
              </div>
            </div>
          );
...
function DepsTab({ rec }: { rec: Request }) {
  const list = Array.from({ length: 2 + rec.dependencies }, (_, i) => {
    ...
    return {
      id: `${rec.id}-d-${i}`,
      dependency: pick(["Trust Relationship", "Isolation Policy", "Shared Resource Policy", "Compliance Assignment"], m),
  ...
  const cols: Column<(typeof list)[number]>[] = [
    ...
    { key: "type", header: "Type", render: (r) => r.type },
    { key: "status", header: "Status", render: (r) => <span style={{ color: r.status === "Satisfied" ? T.success : T.danger }}>{r.status}</span> },
  ...
  return (
    <>
      <div style={{ fontSize: 12.5, color: T.textMuted, marginBottom: 10, display: "flex", alignItems: "center", gap: 8 }}>
        Request → Required Trust → Required Policy → Required Resource. <SampleTag />
      </div>
      <DirectoryTable columns={cols} rows={list} />
    </>
  );
...
function SecurityTab({ rec }: { rec: Request }) {
  const checks = ["Least Privilege", "Isolation Validation", "Conditional Access", "Trust Validation", "Risk Analysis", "Policy Compliance"];
  const list = checks.map((c) => {
    const m = hashId(rec.id + c);
    const result = pick(["Passed", "Passed", "Attention"], m);
    return { id: c, control: c, status: result, severity: result === "Passed" ? "—" : pick(["Medium", "High"], m), recommendation: result === "Passed" ? "Maintain" : "Scope down" };
  });
  const cols: Column<(typeof list)[number]>[] = [
...
function ComplianceTab({ rec }: { rec: Request }) {
  const list = FRAMEWORKS.map((f) => {
    ...
    return { id: f, framework: f, status: pick(["Compliant", "Compliant", "Gap"], m), controls: 2 + (m % 10), violations: m % 3 };
  });
  const cols: Column<(typeof list)[number]>[] = [
...
function ActivityTab({ rec }: { rec: Request }) {
  const events = ["Request Created", "Request Submitted", "Approval Granted", "Approval Rejected", "Request Modified", "Resources Granted", "Request Closed"];
  return (
    <>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
        <Select label="Actor" value="" onChange={() => {}} options={[{ value: "", label: "Actor: All" }, ...APPROVERS.map((o) => ({ value: o, label: o }))]} />
      ...
      {events.map((e, i) => (
        <div key={e} style={{ display: "flex", gap: 12, padding: "10px 0", borderBottom: `1px solid ${T.border}` }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11.5, color: T.textMuted }}>{pick([...REQUESTERS, ...APPROVERS], hashId(rec.id) + i)} · {pick(["5 min", "2 h", "yesterday", "3 days"], i)} ago</div>
          </div>
        </div>
      ))}
    </>
...
function AuditTab() {
  const events = ["Request Created", "Approval Granted", "Approval Rejected", "Policy Validation", "Security Review", "Compliance Review", "Request Cancelled", "Request Closed"];
  return (
    <>
      ...
      {events.map((e, i) => (
        <StatRow key={e} label={e} value={`${pick(APPROVERS, i)} · 2026-06-${(10 + i).toString().padStart(2, "0")}`} tone="ok" sample />
      ))}
    </>
  );
}
