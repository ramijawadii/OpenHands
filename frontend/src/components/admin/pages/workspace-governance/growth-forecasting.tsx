...
const SAMPLE_FORECASTS: Forecast[] = FC_SEED.map(
...
export function GrowthForecastingView() {
  ...
  const predictedGrowth = Math.round(
    records.reduce((a, r) => a + r.growth, 0) / records.length,
  );
  const accuracy = Math.round(
    records.reduce((a, r) => a + r.confidence, 0) / records.length,
  );
  ...
  const fastest =
    [...records].sort((a, b) => b.growth - a.growth)[0]?.workspace ?? "—";
  const bottlenecks = records.filter(
    (r) => r.risk === "Critical" || r.risk === "High",
  ).length;
  const infraRisk = Math.round((bottlenecks / records.length) * 100);
  const optOpportunities = records.filter(
    (r) => r.trend === "Declining" || r.trend === "Flat",
  ).length;
  ...
  const cols: Column<Forecast>[] = [
    ...
    {
      key: "predicted",
      header: "Predicted Usage",
      sortValue: (r) => r.predicted,
      render: (r) => (
    ...
    },
    {
      ...
      render: (r) => (
        <span style={{ color: r.growth >= 40 ? T.danger : T.textNav }}>
          +{r.growth}%
        </span>
      ),
    },
    {
      key: "forecastDate",
      header: "Forecast",
      sortValue: (r) => r.forecastDate,
      render: (r) => r.forecastDate,
    },
    {
      key: "risk",
      header: "Risk",
      sortValue: (r) => RISK_ORDER[r.risk],
      render: (r) => <RiskBadge risk={r.risk} />,
    },
  ];
      ...
      <DiscoveryListView
        ...
        pills={[
          ...
          },
          {
            ...
            value: fHorizon,
            onChange: setFHorizon,
            options: facet(records.map((r) => r.forecastDate)),
          },
          {
            key: "riskLevel",
            label: "Risk Level",
        ...
        presets={[{ label: "All growth forecasting", onApply: clearFilters }]}
        filterRightSlot={
        ...
        bulkActions={(ids, clear) => (
          <>
            <HeaderButton icon={<GitCompare size={13} />} onClick={clear}>
              Compare ({ids.length})
            </HeaderButton>
        ...
        <Section title="General">
          <KVGrid
            items={[
              {
                k: "Forecast Name",
                v: `${rec.resource} — ${rec.forecastDate}`,
              },
        ...
        <Section title="Statistics" sample>
          <KVGrid
            ...
            items={[
              ...
              {
                k: "Projected Usage",
                v: `${rec.predicted} ${rec.unit}`,
                sample: true,
              },
              {
                k: "Capacity Remaining",
                v: `${Math.max(0, rec.predicted - rec.current)} ${rec.unit} gap`,
                sample: true,
              },
              {
                k: "Forecast Confidence",
                v: `${rec.confidence}%`,
                sample: true,
              },
              {
                k: "Predicted Cost",
                v: `$${rec.predictedCost}k`,
                sample: true,
              },
              {
                k: "Risk Score",
                v: <RiskBadge risk={rec.risk} />,
                sample: true,
              },
            ]}
          />
  ...
  return (
    <Section title="Prediction — future predicted growth" sample>
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ fontSize: 11.5, color: T.textMuted, marginBottom: 4 }}>
          Predicted capacity ({rec.forecastDate})
        </div>
        <div
          style={{
            position: "relative",
        ...
        >
          <div
            style={{
              ...
              bottom: 0,
              width: "100%",
              background: rec.growth >= 40 ? T.danger : T.warning,
              borderRadius: 99,
            }}
          />
        </div>
      ...
      <KVGrid
        items={[
          ...
          {
            k: "Capacity Exhaustion",
            v: `~${rec.daysToExhaustion} days`,
            sample: true,
          },
          { k: "Expansion Date", v: rec.forecastDate, sample: true },
          {
            k: "Confidence Interval",
            v: `±${100 - rec.confidence}%`,
            sample: true,
          },
        ]}
      />
...
function CapacityTab({ rec }: { rec: Forecast }) {
  ...
  const list = cats.map((c) => {
    ...
    return {
      ...
      current: cur,
      predicted: pred,
      required: Math.round(pred * 1.1),
      gap: Math.round(pred * 1.1 - cur),
    };
  });
  const cols: Column<(typeof list)[number]>[] = [
    ...
    { key: "required", header: "Required", render: (r) => r.required },
    {
      key: "gap",
      header: "Gap",
      render: (r) => <span style={{ color: T.warning }}>+{r.gap}</span>,
    },
  ];
  return (
    <>
    ...
    <Section title="Cost projection — future cloud costs" sample>
      <KVGrid
        items={[
          ...
          {
            k: "Projected Budget",
            v: `$${Math.round(rec.predictedCost * 12 * 1.1)}k`,
            sample: true,
          },
          {
            k: "Cost Per Workspace",
            v: `$${rec.predictedCost}k`,
            sample: true,
          },
        ]}
  ...
  const list = recs
    .map((r) => {
      ...
      return {
        id: r,
        recommendation: r,
        impact: pick(["High", "Medium", "Low"], m),
        priority: pick(["High", "Medium"], m),
        cost: `$${1 + (m % 20)}k`,
        savings: `$${1 + (m % 8)}k`,
      };
    })
