...
export function SharedResourcePoliciesView() {
  ...
  const protectedResources = records.reduce(
    (a, r) => a + r.assignedResources,
    0,
  );
  ...
  const cols: Column<Policy>[] = [
    ...
    },
    {
      ...
      sortValue: (r) => r.assignedResources,
      render: (r) => r.assignedResources,
    },
    {
      key: "enforcement",
      header: "Enforcement",
  ...
  return (
    <>
      ...
      <DiscoveryListView
        ...
        presets={[
          { label: "All shared resource policies", onApply: clearFilters },
        ]}
        filterRightSlot={
        ...
        <Section title="General">
          <KVGrid
            items={[
              ...
              {
                k: "Enforcement Mode",
                v: <EnforcementPill status={rec.enforcement} />,
              },
      ...
      {sub === "statistics" && (
        <Section title="Statistics" sample>
          <KVGrid
            ...
            items={[
              {
                k: "Assigned Resources",
                v: rec.assignedResources,
                sample: true,
              },
  ...
  return (
    <Section title="Scope — where the policy applies" sample>
      <div style={{ fontSize: 12.5, color: T.textMuted, marginBottom: 10 }}>
        Organization → Workspace → Resource → Policy.
      </div>
      {SCOPES.map((s) => (
        <StatRow
          key={s}
...
function RulesTab() {
  ...
  return (
    <Section
      title="Rules — policy logic (Condition · Operator · Value · Action)"
      sample
    >
      <div
      ...
      >
        ...
        <Select
          label="Value"
          value="Approved Workspaces"
          onChange={() => {}}
          options={[
            { value: "Approved Workspaces", label: "Approved Workspaces" },
          ]}
        />
        <Select
          label="Action"
...
function AssignmentsTab({ rec }: { rec: Policy }) {
  const list = Array.from(
    { length: Math.min(8, rec.assignedResources) },
    (_, i) => {
...
function SecurityTab() {
  ...
  const list = controls.map((c) => {
    ...
    return {
      id: c,
      control: c,
      required: m % 3 === 0 ? "Optional" : "Required",
    ...
    <>
      ...
      {list.length ? (
        ...
        <EmptyState
          ...
          title="No exceptions"
          hint="No approved exceptions to this policy."
        />
      )}
    </>
  );
    ...
    <Section title="Simulation — test policy impact before deployment" sample>
      ...
      >
        ...
        <Select
          ...
          value="Payments API"
          onChange={() => {}}
          options={[{ value: "Payments API", label: "Payments API" }]}
        />
        <Select
          label="Resource"
          value="Central Secrets"
          onChange={() => {}}
          options={[{ value: "Central Secrets", label: "Central Secrets" }]}
        />
        <Select
          label="Permission"
          value="Read"
          onChange={() => {}}
          options={["Read", "Write", "Admin"].map((p) => ({
            value: p,
            label: p,
          }))}
        />
      ...
      {ran ? (
        <>
          ...
          <StatRow
            label="Decision"
            value="Route to Workspace Owner + Security"
            sample
          />
        </>
      ) : (
