...
type ResourceType = "Application" | "Database" | "Cloud Account" | "Kubernetes" | "Network" | "Storage" | "AI Service" | "SaaS";
type OwnershipStatus = "Owned" | "Orphaned" | "Under Review" | "Pending Transfer";
type Environment = "Production" | "Pre-production" | "Development" | "Sandbox";

const RES_TYPES: ResourceType[] = ["Application", "Database", "Cloud Account", "Kubernetes", "Network", "Storage", "AI Service", "SaaS"];
const ENVIRONMENTS: Environment[] = ["Production", "Pre-production", "Development", "Sandbox"];
...
const BUSINESS_OWNERS = ["Payments Director", "Retail VP", "Data Lead", "Identity Owner", "Analytics Lead"];
const TECH_OWNERS = ["Platform Team", "Cloud Team", "Data Team", "Security Team", "App Team"];
const OWNER_TYPES = ["Business Owner", "Technical Owner", "Security Owner", "Compliance Owner", "Service Owner", "Custodian"];
...
export function ResourceOwnershipView() {
  ...
  const hasFilters = !!(search || fType || fProvider || fWs || fBu || fEnv || fStatus || fDept);
  const clearFilters = () => {
    setSearch("");
    setFType("");
    setFProvider("");
  ...
  const businessOwners = new Set(records.filter((r) => r.businessOwner !== "—").map((r) => r.businessOwner)).size;
  const technicalOwners = new Set(records.filter((r) => r.technicalOwner !== "—").map((r) => r.technicalOwner)).size;
  const reviews = records.filter((r) => r.status === "Under Review").length;
  const pendingRequests = records.filter((r) => r.status === "Pending Transfer").length;
  const coverage = Math.round((owned / records.length) * 100);

  const toolbar: CommandItem[] = [
    { key: "assign", label: "Assign Owner", icon: <UserPlus size={15} />, onClick: () => navigate("/admin/workspace-governance?tab=inheritance") },
    { key: "transfer", label: "Transfer Ownership", icon: <ArrowLeftRight size={15} />, disabled: true },
    { key: "remove", label: "Remove Owner", icon: <UserMinus size={15} />, disabled: true },
    { key: "bulk", label: "Bulk Assignment", icon: <Users size={15} />, disabled: true },
    { key: "export", label: "Export", icon: <Download size={15} />, disabled: true },
    { key: "review", label: "Launch Ownership Review", icon: <ClipboardCheck size={15} />, disabled: true },
    { key: "orphan", label: "Detect Orphaned", icon: <ScanSearch size={15} />, disabled: true },
    { key: "validate", label: "Validate Ownership", icon: <ClipboardCheck size={15} />, disabled: true },
    { key: "report", label: "Generate Report", icon: <FileText size={15} />, disabled: true },
    { key: "sync", label: "Synchronize", icon: <RefreshCcw size={15} />, onClick: () => setSelId(null) },
    { key: "import", label: "Import Ownership", icon: <Upload size={15} />, disabled: true },
  ];
  ...
  const cols: Column<Resource>[] = [
    ...
    { key: "businessOwner", header: "Business Owner", sortValue: (r) => r.businessOwner, render: (r) => r.businessOwner },
    { key: "technicalOwner", header: "Technical Owner", sortValue: (r) => r.technicalOwner, render: (r) => r.technicalOwner },
    ...
    { key: "status", header: "Ownership Status", sortValue: (r) => r.status, render: (r) => <StatusBadge status={r.status} /> },
  ];

  return (
    <>
      <PostureGrid>
        <PostureCard title="Owned Resources" value={owned} tone="ok" sub={<>With valid ownership <SampleTag /></>} />
        <PostureCard title="Orphaned Resources" value={orphaned} tone={orphaned > 0 ? "danger" : "ok"} sub={<>No valid owner <SampleTag /></>} />
        <PostureCard title="Business Owners" value={businessOwners} tone="ok" sub={<>Accountable leaders <SampleTag /></>} />
        <PostureCard title="Technical Owners" value={technicalOwners} tone="ok" sub={<>Operational teams <SampleTag /></>} />
        <PostureCard title="Ownership Reviews" value={reviews} tone={reviews > 0 ? "warn" : "ok"} sub={<>Certification due <SampleTag /></>} />
        <PostureCard title="Pending Requests" value={pendingRequests} tone={pendingRequests > 0 ? "warn" : "ok"} sub={<>Transfers awaiting <SampleTag /></>} />
        <PostureCard title="Ownership Violations" value={orphaned} tone={orphaned > 0 ? "warn" : "ok"} sub={<>Failing governance rules <SampleTag /></>} />
        <PostureCard title="Ownership Coverage" value={`${coverage}%`} tone={coverage >= 90 ? "ok" : "warn"} sub={<>Resources with owners <SampleTag /></>} />
      </PostureGrid>
      ...
      >
        ...
        >
          ...
          <Select label="Department" value={fDept} onChange={setFDept} options={facet(records.map((r) => r.department))} />
          <Select label="Environment" value={fEnv} onChange={setFEnv} options={facet(records.map((r) => r.environment))} />
          <Select label="Ownership State" value={fStatus} onChange={setFStatus} options={facet(records.map((r) => r.status))} />
        </FilterBar>
        ...
        <DirectoryTable
          ...
          bulkActions={(ids, clear) => (
            <>
              <HeaderButton icon={<UserPlus size={13} />} onClick={clear}>Assign Owner ({ids.length})</HeaderButton>
              <HeaderButton icon={<ArrowLeftRight size={13} />} onClick={clear}>Transfer</HeaderButton>
              <HeaderButton icon={<ClipboardCheck size={13} />} onClick={clear}>Launch Review</HeaderButton>
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <HeaderButton variant="primary" icon={<UserPlus size={14} />} onClick={() => navigate("/admin/workspace-governance?tab=inheritance")}>
              Assign Owner
            </HeaderButton>
          </>
        }
...
function OwnershipDrawer({ rec, onClose }: { rec: Resource; onClose: () => void }) {
    ...
    <SideRailDrawer
      ...
      footer={
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end", width: "100%" }}>
          ...
          <HeaderButton icon={<ArrowLeftRight size={13} />}>Transfer</HeaderButton>
          <HeaderButton variant="primary" icon={<ClipboardCheck size={13} />}>Launch Review</HeaderButton>
        </div>
      }
    >
        ...
        <Section title="General">
          ...
          {rec.status === "Orphaned" && (
            <div style={{ fontSize: 12.5, color: T.danger, paddingTop: 6, display: "flex", alignItems: "center", gap: 6 }}>
              <AlertTriangle size={14} /> Orphaned {rec.daysWithoutOwner} days — governance violation.
            </div>
          )}
        </Section>
      )}
...
function OwnershipTab({ rec }: { rec: Resource }) {
  const list =
    rec.status === "Orphaned"
      ...
      : OWNER_TYPES.slice(0, 2 + (hashId(rec.id) % 4)).map((otype, i) => {
          ...
          return {
            id: `${rec.id}-o-${i}`,
            owner: otype === "Business Owner" ? rec.businessOwner : otype === "Technical Owner" ? rec.technicalOwner : pick(TECH_OWNERS, m),
            ownershipType: otype,
  ...
  const cols: Column<(typeof list)[number]>[] = [
    ...
    { key: "ownershipType", header: "Ownership Type", render: (r) => r.ownershipType },
  ...
  return (
    <>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
        <HeaderButton icon={<UserPlus size={13} />}>Assign Owner</HeaderButton>
        <HeaderButton icon={<ArrowLeftRight size={13} />}>Transfer Ownership</HeaderButton>
        <HeaderButton variant="danger" icon={<UserMinus size={13} />}>Remove Owner</HeaderButton>
        <SampleTag />
      </div>
      {list.length ? (
        <DirectoryTable columns={cols} rows={list} />
      ) : (
        <EmptyState icon={<AlertTriangle size={18} />} title="Orphaned resource" hint="No valid ownership assigned. Assign a Business Owner and Technical Owner to remediate." />
      )}
    </>
  );
}
...
function DepsTab({ rec }: { rec: Resource }) {
  const list = Array.from({ length: 2 + (hashId(rec.id) % 4) }, (_, i) => {
    ...
    return {
      id: `${rec.id}-d-${i}`,
      dep: pick(["Payments DB", "IAM Role", "Shared VPC", "Redis Cache", "Auth Service"], m),
...
function ComplianceTab({ rec }: { rec: Resource }) {
  ...
  const cols: Column<(typeof list)[number]>[] = [
    ...
    { key: "required", header: "Ownership Required", render: (r) => r.required },
    { key: "review", header: "Review Status", render: (r) => r.review },
    { key: "status", header: "Compliance Status", render: (r) => (
...
function ActivityTab({ rec }: { rec: Resource }) {
  const events = ["Owner Assigned", "Owner Removed", "Ownership Transferred", "Ownership Review", "Governance Updated", "Policy Updated"];
  return (
    <>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
        <Select label="Actor" value="" onChange={() => {}} options={[{ value: "", label: "Actor: All" }, ...TECH_OWNERS.map((o) => ({ value: o, label: o }))]} />
        ...
        <div key={e} style={{ display: "flex", gap: 12, padding: "10px 0", borderBottom: `1px solid ${T.border}` }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11.5, color: T.textMuted }}>
              {pick(TECH_OWNERS, hashId(rec.id) + i)} · {pick(["5 min", "2 h", "yesterday", "3 days"], i)} ago
            </div>
          </div>
        </div>
      ))}
...
function AuditTab() {
  const events = ["Ownership Assigned", "Ownership Updated", "Ownership Removed", "Ownership Transferred", "Ownership Review Completed", "Orphaned Resource Detected", "Ownership Restored"];
  return (
    <>
      ...
      {events.map((e, i) => (
        <StatRow key={e} label={e} value={`${pick(TECH_OWNERS, i)} · 2026-06-${(10 + i).toString().padStart(2, "0")}`} tone="ok" sample />
      ))}
    </>
  );
}
