...
type IdentityType = "User" | "Group" | "Role" | "Service Account" | "Application" | "AI Agent" | "Managed Identity";
type AccessType = "Temporary" | "Permanent" | "Service" | "Human" | "AI Agent" | "Emergency";
type Permission = "Read" | "Write" | "Execute" | "Deploy" | "Admin" | "Owner";
type Status = "Active" | "Pending" | "Expired" | "Revoked";

const IDENTITY_TYPES: IdentityType[] = ["User", "Group", "Role", "Service Account", "Application", "AI Agent", "Managed Identity"];
const ACCESS_TYPES: AccessType[] = ["Temporary", "Permanent", "Service", "Human", "AI Agent", "Emergency"];
const PERMISSIONS: Permission[] = ["Read", "Write", "Execute", "Deploy", "Admin", "Owner"];
const WORKSPACES = ["Payments", "Shared Services", "Retail Web", "Data Lake", "Identity", "Analytics", "Security Ops"];
const RESOURCES = ["Central Secrets", "Shared VPC", "Logging Cluster", "AI Gateway", "Shared Database", "Image Registry", "API Gateway"];
const ENVIRONMENTS = ["Production", "Pre-production", "Development", "Shared"];
const OWNERS = ["Security Team", "Platform Team", "Access Admin", "Workspace Owner"];
const FRAMEWORKS = ["ISO 27001", "SOC 2", "NIST", "PCI DSS", "HIPAA", "CSA CCM"];

const STATUS_TONE: Record<Status, string> = { Active: T.success, Pending: T.warning, Expired: T.textMuted, Revoked: T.danger };
const PERM_TONE: Record<Permission, string> = { Read: T.textMuted, Write: T.accent, Execute: T.accent, Deploy: T.warning, Admin: T.danger, Owner: T.danger };
...
const IDENTITY_NAMES = ["Payments API", "billing-svc", "Data Team", "sre-agent", "Analytics Role", "deploy-bot", "Support Group", "backup-svc", "security-agent", "Mobile App", "etl-svc", "Platform Admins", "audit-reader", "ml-agent", "webhook-svc"];
...
const SAMPLE_ACCESS: Access[] = IDENTITY_NAMES.map((identity, i) => {
  ...
  const status = pick<Status>(["Active", "Active", "Active", "Pending", "Expired", "Revoked"], n);
  return {
    id,
    identity,
...
function PermBadge({ permission }: { permission: Permission }) {
  ...
  return <span style={{ fontSize: 11.5, color: c, border: `1px solid ${c}55`, borderRadius: 99, padding: "2px 9px" }}>{permission}</span>;
}
...
export function CrossWorkspaceAccessView() {
  ...
  const rows = records.filter((r) => {
    ...
    return (
      (!q || r.identity.toLowerCase().includes(q) || r.resource.toLowerCase().includes(q) || r.owner.toLowerCase().includes(q)) &&
  ...
  const hasFilters = !!(search || fSource || fTarget || fIdentity || fAccess || fEnv || fStatus);
  const clearFilters = () => {
    setSearch("");
    setFSource("");
    setFTarget("");
  ...
  const toolbar: CommandItem[] = [
    { key: "grant", label: "Grant Access", icon: <KeyRound size={15} />, onClick: () => navigate("/admin/workspace-governance?tab=cross") },
    { key: "request", label: "Request Access", icon: <Send size={15} />, disabled: true },
    { key: "approve", label: "Approve Access", icon: <Check size={15} />, disabled: true },
    { key: "revoke", label: "Revoke Access", icon: <Ban size={15} />, disabled: true },
    { key: "extend", label: "Extend Access", icon: <Clock size={15} />, disabled: true },
    { key: "review", label: "Launch Review", icon: <ClipboardCheck size={15} />, disabled: true },
    { key: "excessive", label: "Detect Excessive Access", icon: <ShieldAlert size={15} />, disabled: true },
    ...
    { key: "refresh", label: "Refresh Permissions", icon: <RefreshCcw size={15} />, onClick: () => setSelId(null) },
  ...
  const cols: Column<Access>[] = [
    {
      ...
      render: (r) => (
        <span style={{ color: T.textPrimary, display: "inline-flex", alignItems: "center", gap: 8 }}>
          ...
          {r.identity}
          <span style={{ fontSize: 10.5, color: T.textMuted }}>· {r.identityType}</span>
        </span>
      ),
    },
    { key: "route", header: "Source → Target", render: (r) => (
      <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12, color: T.textNav }}>
    ...
    { key: "permission", header: "Permission", sortValue: (r) => r.permission, render: (r) => <PermBadge permission={r.permission} /> },
    { key: "expiration", header: "Expiration", sortValue: (r) => r.expiration, render: (r) => r.expiration },
  ...
  return (
    <>
      <PostureGrid>
        <PostureCard title="Active Access Grants" value={activeGrants} tone="ok" sub={<>Effective now <SampleTag /></>} />
        <PostureCard title="Pending Requests" value={pending} tone={pending > 0 ? "warn" : "ok"} sub={<>Awaiting approval <SampleTag /></>} />
        <PostureCard title="Temporary Grants" value={temporary} tone="ok" sub={<>Time-limited access <SampleTag /></>} />
        <PostureCard title="Expired Grants" value={expired} tone="ok" sub={<>No longer valid <SampleTag /></>} />
        <PostureCard title="Shared Resources" value={sharedResources} tone="ok" sub={<>Accessed cross-workspace <SampleTag /></>} />
        <PostureCard title="Privileged Access" value={privileged} tone={privileged > 0 ? "warn" : "ok"} sub={<>Admin/Owner grants <SampleTag /></>} />
        <PostureCard title="Policy Violations" value={violations} tone={violations > 0 ? "warn" : "ok"} sub={<>Failing governance <SampleTag /></>} />
        <PostureCard title="Access Reviews" value={reviews} tone="ok" sub={<>Certification scope <SampleTag /></>} />
      </PostureGrid>
      ...
      >
        ...
        >
          <Select label="Source Workspace" value={fSource} onChange={setFSource} options={facet(records.map((r) => r.source))} />
          <Select label="Target Workspace" value={fTarget} onChange={setFTarget} options={facet(records.map((r) => r.target))} />
          <Select label="Identity Type" value={fIdentity} onChange={setFIdentity} options={facet(records.map((r) => r.identityType))} />
          <Select label="Access Type" value={fAccess} onChange={setFAccess} options={facet(records.map((r) => r.accessType))} />
        ...
        <DirectoryTable
          ...
          bulkActions={(ids, clear) => (
            <>
              ...
              <HeaderButton icon={<Ban size={13} />} onClick={clear}>Revoke</HeaderButton>
              <HeaderButton icon={<Clock size={13} />} onClick={clear}>Extend</HeaderButton>
              <HeaderButton icon={<ClipboardCheck size={13} />} onClick={clear}>Review</HeaderButton>
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <HeaderButton variant="primary" icon={<KeyRound size={14} />} onClick={() => navigate("/admin/workspace-governance?tab=cross")}>
              Grant Access
            </HeaderButton>
          </>
        }
...
const DRAWER_TABS = [
  ...
  { id: "approval", label: "Approval Workflow", icon: <GitPullRequestArrow size={13} /> },
        ...
        <Section title="General">
          <KVGrid
            items={[
              ...
              { k: "Purpose", v: `${rec.permission} access to ${rec.resource}`, sample: true },
        ...
        <Section title="Statistics" sample>
          <KVGrid
            ...
            items={[
              ...
              { k: "Inherited Access", v: rec.permission === "Admin" ? "Yes" : "No", sample: true },
...
function IdentityTab({ rec }: { rec: Access }) {
  const list = [{ id: rec.id, identity: rec.identity, type: rec.identityType, workspace: rec.source, owner: rec.owner, status: "Active" }];
...
function PermissionsTab({ rec }: { rec: Access }) {
  ...
  const cols: Column<(typeof list)[number]>[] = [
    { key: "permission", header: "Permission", render: (r) => <PermBadge permission={r.permission as Permission} /> },
...
function ApprovalTab({ rec }: { rec: Access }) {
  const stages = ["Request", "Manager Approval", "Workspace Owner", "Security Approval", "Granted"];
  return (
      ...
      {stages.map((s, i) => {
        ...
        return (
          <div key={s} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0" }}>
            <span style={{ width: 20, height: 20, borderRadius: "50%", background: done ? T.success : T.border, color: "#fff", fontSize: 11, display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              {done ? "✓" : i + 1}
            </span>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, color: T.textPrimary }}>{s}</div>
              <div style={{ fontSize: 11.5, color: T.textMuted }}>{done ? `${pick(OWNERS, hashId(rec.id) + i)} · approved` : "Pending"}</div>
            </div>
          </div>
        );
      })}
...
function EffectiveTab({ rec }: { rec: Access }) {
  const layers = ["User", "Group", "Role", "Workspace Policy", "Trust Relationship", "Cross-Workspace Access", "Final Permission"];
  return (
    <Section title="Effective access — final permission after inheritance" sample>
      <div style={{ fontSize: 12.5, color: T.textMuted, marginBottom: 12 }}>The evaluation chain producing the final permission.</div>
      {layers.map((l, i) => (
        <div key={l} style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 0" }}>
          <div style={{ width: 4, height: 24, background: i === layers.length - 1 ? T.success : T.border, borderRadius: 2 }} />
          <div style={{ fontSize: 13, color: i === layers.length - 1 ? T.textPrimary : T.textMuted, fontWeight: i === layers.length - 1 ? 600 : 400 }}>{l}</div>
          {i === layers.length - 1 && <span style={{ marginLeft: "auto" }}><PermBadge permission={rec.permission} /></span>}
        </div>
      ))}
    </Section>
  );
...
function SecurityTab({ rec }: { rec: Access }) {
  const controls = ["Conditional Access", "MFA Requirement", "Least Privilege", "Zero Trust Validation", "Session Controls"];
  ...
  const cols: Column<(typeof list)[number]>[] = [
    ...
    { key: "status", header: "Status", render: (r) => <span style={{ color: r.status === "Passed" ? T.success : T.warning }}>{r.status}</span> },
  ...
  return (
    <>
      <Section title="Zero-trust security posture" sample>
        <StatRow label="Risk Score" value={`${rec.policyViolations * 12 + (rec.privileged ? 30 : 0)}/100`} tone={rec.privileged ? "warn" : "ok"} sample />
        <StatRow label="Least Privilege" value={rec.privileged ? "Review — privileged" : "Compliant"} tone={rec.privileged ? "warn" : "ok"} sample />
      </Section>
      <DirectoryTable columns={cols} rows={list} />
    </>
  );
...
function ComplianceTab({ rec }: { rec: Access }) {
  const list = FRAMEWORKS.map((f) => {
    ...
    return { id: f, framework: f, status: pick(["Compliant", "Compliant", "Gap"], m), violations: m % 3, controls: 2 + (m % 10) };
  });
  const cols: Column<(typeof list)[number]>[] = [
...
function ActivityTab({ rec }: { rec: Access }) {
  const events = ["Access Requested", "Access Granted", "Access Modified", "Permission Updated", "Access Extended", "Access Revoked", "Review Completed"];
  return (
    <>
...
function AuditTab() {
  const events = ["Access Requested", "Access Approved", "Permission Granted", "Permission Modified", "Permission Revoked", "Emergency Access Used", "Review Completed"];
  return (
    <>
