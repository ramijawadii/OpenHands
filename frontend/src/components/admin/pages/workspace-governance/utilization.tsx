...
const STATUS_TONE: Record<Status, string> = {
  Healthy: T.success,
  Underutilized: T.warning,
  Overutilized: T.danger,
};
const EFF_TONE: Record<Efficiency, string> = {
  Excellent: T.success,
  Good: T.success,
  Fair: T.warning,
  Poor: T.danger,
...
const SAMPLE_UTIL: Util[] = UTIL_SEED.map(({ resource, type, unit }, i) => {
  ...
  const status: Status =
    util >= 0.9 ? "Overutilized" : util <= 0.35 ? "Underutilized" : "Healthy";
...
export function UtilizationView() {
  ...
  const overall = Math.round(
    (records.reduce((a, r) => a + r.used / r.allocated, 0) / records.length) *
      100,
  );
  ...
  const effScore = Math.round(
    (records.filter(
      (r) => r.efficiency === "Excellent" || r.efficiency === "Good",
    ).length /
      records.length) *
      100,
  );
  ...
  const toolbar: CommandItem[] = [
    ...
    },
    {
      ...
      label: "Detect Waste",
      icon: <Trash2 size={15} />,
      onClick: () => setFStatus("Underutilized"),
    },
    {
      key: "capacity",
  ...
  const cols: Column<Util>[] = [
    ...
    },
    {
      key: "utilization",
      header: "Utilization",
      sortValue: (r) => r.used / r.allocated,
      render: (r) => (
        <UsageBar
          pct={(r.used / r.allocated) * 100}
          tone={r.used / r.allocated <= 0.35 ? T.warning : undefined}
        />
      ),
    },
    {
      ...
      <DiscoveryListView
        ...
        pills={[
          ...
          },
          {
            ...
            value: fType,
            onChange: setFType,
            options: facet(records.map((r) => r.resType)),
          },
          {
        ...
        presets={[{ label: "All utilization", onApply: clearFilters }]}
        filterRightSlot={
        ...
        bulkActions={(ids, clear) => (
          <>
            <HeaderButton icon={<Sparkles size={13} />} onClick={clear}>
              Optimize ({ids.length})
            </HeaderButton>
        ...
        <Section title="Statistics" sample>
          <KVGrid
            ...
            items={[
              ...
              {
                k: "Average Utilization",
                v: `${Math.round((rec.average / rec.allocated) * 100)}%`,
                sample: true,
              },
              {
                k: "Peak Utilization",
                v: `${Math.round((rec.peak / rec.allocated) * 100)}%`,
                sample: true,
              },
              {
                k: "Idle Capacity",
              ...
              },
              {
                ...
                v: `${Math.max(0, rec.allocated - rec.used)} ${rec.unit}`,
                sample: true,
              },
              {
                k: "Efficiency Score",
                v: <EffBadge efficiency={rec.efficiency} />,
                sample: true,
              },
            ]}
          />
...
function MetricsTab({ rec }: { rec: Util }) {
  ...
  const list = metrics.map((m) => {
    ...
    return {
      ...
      util: (Math.min(used, alloc) / alloc) * 100,
      health:
        used / alloc >= 0.9
          ? "Overutilized"
          : used / alloc <= 0.35
  ...
  return (
    <Section title="Capacity analysis — resource efficiency" sample>
      <StatRow
        ...
        value={`${rec.allocated} ${rec.unit}`}
        sample
      />
      <StatRow
        label="Consumed Capacity"
        value={`${rec.used} ${rec.unit}`}
        sample
      />
      <StatRow
        label="Unused Capacity"
        value={`${rec.idle} ${rec.unit}`}
        tone={rec.idle > 0 ? "warn" : "ok"}
        sample
      />
      <StatRow
        label="Waste"
        value={
          rec.status === "Underutilized" ? `~$${rec.savings}k/mo` : "Minimal"
        }
        tone={rec.status === "Underutilized" ? "warn" : "ok"}
        sample
      />
      <StatRow
        label="Utilization"
        value={
          <UsageBar pct={util} tone={util <= 0.35 ? T.warning : undefined} />
        }
        sample
      />
      <StatRow
        ...
        value={
          rec.status === "Underutilized"
            ? `${Math.round(rec.allocated * 0.7)} ${rec.unit}`
            : `${rec.allocated} ${rec.unit}`
        }
        sample
      />
    </Section>
            ...
            >
              <div
                style={{
                  ...
                  height: `${b}%`,
                  background:
                    b >= 85 ? T.danger : b <= 40 ? T.warning : T.accent,
...
function OptimizationTab({ rec }: { rec: Util }) {
  ...
  const list = opts
    .map((o) => {
      ...
      return {
        ...
        recommendation: o,
        savings: `$${1 + (m % 12)}k/mo`,
        priority: pick(["High", "Medium", "Low"], m),
        impact: pick(["Low", "Medium"], m),
        risk: pick(["Low", "Low", "Medium"], m),
      };
    })
  ...
  const cols: Column<(typeof list)[number]>[] = [
    ...
    },
    {
      ...
      render: (r) => <span style={{ color: T.success }}>{r.savings}</span>,
    },
  ...
  return (
    <>
      ...
      >
        <HeaderButton variant="primary" icon={<Sparkles size={13} />}>
          Run Optimization
        </HeaderButton>
        <SampleTag />
      </div>
  ...
  return (
    <>
      ...
      <Section title="Utilization forecast" sample>
        <StatRow
          label="Growth Trend"
          value={ran ? "+8% / month" : "Run forecast"}
          sample
        />
        <StatRow
          ...
          value={
            ran
              ? `${Math.min(100, Math.round((rec.used / rec.allocated) * 100) + 20)}% in 90d`
              : "—"
          }
          sample
        />
        <StatRow
          ...
          value={
            ran
              ? rec.status === "Overutilized"
                ? "~40 days"
                : "Not projected"
              : "—"
          }
          tone={rec.status === "Overutilized" ? "warn" : "ok"}
          sample
        />
        <StatRow
          label="Recommended Capacity"
          value={ran ? `${Math.round(rec.allocated * 1.2)} ${rec.unit}` : "—"}
          sample
        />
