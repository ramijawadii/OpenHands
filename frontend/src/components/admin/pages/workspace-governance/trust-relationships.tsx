...
const TRUST_TYPES: TrustType[] = ["Identity Trust", "Resource Sharing", "Application Trust", "Service Trust", "Network Trust", "Secrets Trust", "AI Platform Trust", "Federation Trust"];
const SCOPES: AccessScope[] = ["Read Only", "Read/Write", "Scoped", "Admin"];
const WORKSPACES = ["Payments", "Shared Services", "Retail Web", "Data Lake", "Identity", "Analytics", "Security Ops", "Central Logging"];
...
const OWNERS = ["Governance Admin", "Security Team", "Platform Team", "Workspace Owner"];
...
const SAMPLE_TRUSTS: Trust[] = Array.from({ length: 15 }, (_, i) => {
  ...
  const status = pick<Status>(["Active", "Active", "Active", "Pending", "Suspended", "Expiring", "Revoked"], n);
  return {
    id,
    name: `${source} → ${target}`,
    source,
...
export function TrustRelationshipsView() {
  ...
  const rows = records.filter((r) => {
    ...
    return (
      (!q || r.name.toLowerCase().includes(q) || r.owner.toLowerCase().includes(q) || r.trustType.toLowerCase().includes(q)) &&
  ...
  const hasFilters = !!(search || fSource || fTarget || fType || fEnv || fStatus || fBu);
  const clearFilters = () => {
    setSearch("");
    setFSource("");
    setFTarget("");
  ...
  const connectedWorkspaces = new Set([...records.map((r) => r.source), ...records.map((r) => r.target)]).size;
  ...
  const toolbar: CommandItem[] = [
    { key: "create", label: "Create Trust", icon: <Plus size={15} />, onClick: () => navigate("/admin/workspace-governance?tab=cross") },
    { key: "request", label: "Request Trust", icon: <Send size={15} />, disabled: true },
    { key: "approve", label: "Approve Trust", icon: <Check size={15} />, disabled: true },
    { key: "reject", label: "Reject Trust", icon: <X size={15} />, disabled: true },
    { key: "suspend", label: "Suspend Trust", icon: <PauseCircle size={15} />, disabled: true },
    { key: "revoke", label: "Revoke Trust", icon: <Ban size={15} />, disabled: true },
  ...
  const cols: Column<Trust>[] = [
    ...
    { key: "trustType", header: "Trust Type", sortValue: (r) => r.trustType, render: (r) => r.trustType },
    { key: "scope", header: "Access Scope", sortValue: (r) => r.scope, render: (r) => r.scope },
  ...
  return (
    <>
      <PostureGrid>
        <PostureCard title="Active Trusts" value={activeTrusts} tone="ok" sub={<>In effect now <SampleTag /></>} />
        <PostureCard title="Pending Requests" value={pending} tone={pending > 0 ? "warn" : "ok"} sub={<>Awaiting approval <SampleTag /></>} />
        <PostureCard title="Expired Trusts" value={expired} tone="ok" sub={<>No longer valid <SampleTag /></>} />
        <PostureCard title="Revoked Trusts" value={revoked} tone="ok" sub={<>Permanently terminated <SampleTag /></>} />
        <PostureCard title="Connected Workspaces" value={connectedWorkspaces} tone="ok" sub={<>Participating in trust <SampleTag /></>} />
        <PostureCard title="Shared Resources" value={sharedResources} tone="ok" sub={<>Exposed via trust <SampleTag /></>} />
        <PostureCard title="Policy Violations" value={violations} tone={violations > 0 ? "warn" : "ok"} sub={<>Failing validation <SampleTag /></>} />
        <PostureCard title="Trust Reviews" value={reviews} tone={reviews > 0 ? "warn" : "ok"} sub={<>Certifications due <SampleTag /></>} />
      </PostureGrid>
      ...
      >
        ...
        >
          ...
          <Select label="Trust Type" value={fType} onChange={setFType} options={facet(records.map((r) => r.trustType))} />
        ...
        <DirectoryTable
          ...
          bulkActions={(ids, clear) => (
            <>
              ...
              <HeaderButton icon={<PauseCircle size={13} />} onClick={clear}>Suspend</HeaderButton>
...
const DRAWER_TABS = [
  ...
  { id: "federation", label: "Identity Federation", icon: <Fingerprint size={13} /> },
    ...
    <SideRailDrawer
      ...
      footer={
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end", width: "100%" }}>
          ...
          <HeaderButton icon={<ClipboardCheck size={13} />}>Launch Review</HeaderButton>
      ...
      {sub === "statistics" && (
        <Section title="Statistics" sample>
          <KVGrid
            cols={3}
            items={[
              { k: "Connected Resources", v: rec.connectedResources, sample: true },
  ...
  return (
    <>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
        ...
        <HeaderButton icon={<KeyRound size={13} />}>Transfer Ownership</HeaderButton>
        <SampleTag />
      </div>
...
function ScopeTab({ rec }: { rec: Trust }) {
  const scopes = ["Identity", "Groups", "Roles", "Applications", "Cloud Resources", "Storage", "Networks", "Secrets", "AI Resources", "Monitoring", "Logs", "Policies"];
...
function PoliciesTab() {
  const items = ["Trust Policy", "Approval Policy", "Security Policy", "Compliance Policy", "Isolation Policy", "Expiration Policy"];
  return (
    <>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
        <HeaderButton icon={<ListChecks size={13} />}>Assign Policy</HeaderButton>
        <HeaderButton variant="danger" icon={<X size={13} />}>Remove Policy</HeaderButton>
        <SampleTag />
      </div>
...
function ResourcesTab({ rec }: { rec: Trust }) {
  const list = Array.from({ length: Math.min(8, rec.connectedResources) }, (_, i) => {
    const m = hashId(`${rec.id}-r-${i}`);
    return {
      id: `${rec.id}-r-${i}`,
      resource: pick(["Central Secrets", "Shared VPC", "Logging Cluster", "AI Gateway", "Image Registry", "Shared DB"], m),
      type: pick(["Secrets", "Network", "Logging", "AI", "Registry", "Database"], m),
...
function FederationTab({ rec }: { rec: Trust }) {
  const providers = ["Microsoft Entra ID", "AWS IAM", "Google IAM", "OIDC", "SAML"];
  ...
  const cols: Column<(typeof list)[number]>[] = [
    ...
    { key: "authentication", header: "Authentication", render: (r) => r.authentication },
    { key: "synchronization", header: "Synchronization", render: (r) => r.synchronization },
    { key: "status", header: "Status", render: (r) => r.status },
  ];
  return (
    <>
...
function SecurityTab({ rec }: { rec: Trust }) {
  const controls = ["Encryption", "Authentication", "Authorization", "Audit Logging", "Conditional Access"];
  ...
  const cols: Column<(typeof list)[number]>[] = [
    ...
    { key: "status", header: "Status", render: (r) => <span style={{ color: r.status === "Enabled" ? T.success : T.warning }}>{r.status}</span> },
  ...
  return (
    <>
      <Section title="Security posture" sample>
        <StatRow label="Risk Score" value={`${rec.securityFindings * 8}/100`} tone={rec.securityFindings > 3 ? "warn" : "ok"} sample />
      </Section>
      <DirectoryTable columns={cols} rows={list} />
    </>
  );
...
function ActivityTab({ rec }: { rec: Trust }) {
  const events = ["Trust Created", "Trust Approved", "Policy Updated", "Access Granted", "Access Revoked", "Review Completed", "Trust Suspended"];
  return (
    <>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
...
function AuditTab() {
  const events = ["Trust Created", "Trust Modified", "Trust Approved", "Trust Rejected", "Trust Suspended", "Trust Revoked", "Policy Updated", "Review Completed"];
  return (
    <>
