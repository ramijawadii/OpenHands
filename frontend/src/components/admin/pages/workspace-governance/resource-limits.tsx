...
type LimitType = "Compute" | "Storage" | "Network" | "Kubernetes" | "Database" | "AI" | "Platform Service" | "Custom";
type Enforcement = "enforced" | "monitoring" | "scheduled" | "partially-enforced";
type Status = "Healthy" | "Warning" | "Exceeded";

const LIMIT_TYPES: LimitType[] = ["Compute", "Storage", "Network", "Kubernetes", "Database", "AI", "Platform Service", "Custom"];
...
const OWNERS = ["Capacity Admin", "Platform Team", "Cloud Team", "Workspace Admin"];
const ENFORCE_MODES = ["Monitor Only", "Warn", "Block Provisioning", "Auto Scale", "Require Approval", "Hard Limit"];
...
const SAMPLE_LIMITS: Limit[] = LIMIT_SEED.flatMap(({ resource, type, unit }, i) =>
  [WORKSPACES[i % WORKSPACES.length]].map((workspace) => {
    ...
    const enforcement: Enforcement = status === "Exceeded" ? "enforced" : pick<Enforcement>(["enforced", "monitoring", "partially-enforced", "scheduled"], n);
    return {
      id,
      resource,
      limitType: type,
...
export function ResourceLimitsView() {
  ...
  const healthyWorkspaces = new Set(records.filter((r) => r.status === "Healthy").map((r) => r.workspace)).size;
  const utilization = Math.round((records.reduce((a, r) => a + r.current / r.limit, 0) / records.length) * 100);
  const capacityScore = 100 - Math.round((exceeded / configured) * 100) - warnings;

  const toolbar: CommandItem[] = [
    { key: "create", label: "Create Limit", icon: <Plus size={15} />, onClick: () => navigate("/admin/workspace-governance?tab=capacity") },
    { key: "edit", label: "Edit Limit", icon: <Pencil size={15} />, disabled: true },
    { key: "assign", label: "Assign Limit", icon: <UserCheck size={15} />, disabled: true },
    { key: "clone", label: "Clone Limit", icon: <Copy size={15} />, disabled: true },
    { key: "delete", label: "Delete Limit", icon: <Trash2 size={15} />, disabled: true },
    { key: "validate", label: "Validate Limits", icon: <ClipboardCheck size={15} />, disabled: true },
    { key: "review", label: "Run Capacity Review", icon: <Gauge size={15} />, disabled: true },
    { key: "compare", label: "Compare Limits", icon: <GitCompare size={15} />, disabled: true },
  ...
  const cols: Column<Limit>[] = [
    {
      ...
      render: (r) => (
        <span style={{ color: T.textPrimary, display: "inline-flex", alignItems: "center", gap: 8 }}>
          ...
          {r.resource}
          <span style={{ fontSize: 10.5, color: T.textMuted }}>· {r.limitType}</span>
        </span>
      ),
    },
    ...
    { key: "limit", header: "Limit", sortValue: (r) => r.limit, render: (r) => `${r.limit} ${r.unit}` },
    { key: "utilization", header: "Utilization", sortValue: (r) => r.current / r.limit, render: (r) => <UsageBar pct={(r.current / r.limit) * 100} /> },
  ...
  return (
    <>
      <PostureGrid>
        <PostureCard title="Configured Limits" value={configured} tone="ok" sub={<>Enforced limits <SampleTag /></>} />
        <PostureCard title="Resources Monitored" value={monitored} tone="ok" sub={<>Under limit tracking <SampleTag /></>} />
        <PostureCard title="Exceeded Limits" value={exceeded} tone={exceeded > 0 ? "danger" : "ok"} sub={<>Over ceiling <SampleTag /></>} />
        <PostureCard title="Warning Thresholds" value={warnings} tone={warnings > 0 ? "warn" : "ok"} sub={<>Near ceiling <SampleTag /></>} />
        <PostureCard title="Healthy Workspaces" value={healthyWorkspaces} tone="ok" sub={<>Within all limits <SampleTag /></>} />
        <PostureCard title="Quota Utilization" value={`${utilization}%`} tone={utilization >= 85 ? "warn" : "ok"} sub={<>Average across limits <SampleTag /></>} />
        <PostureCard title="Cloud Spend" value={`$${(120 + (hashId("spend") % 80))}k`} tone="ok" sub={<>Monthly, limit-bound <SampleTag /></>} />
        <PostureCard title="Capacity Score" value={`${Math.max(0, capacityScore)}%`} tone={capacityScore >= 80 ? "ok" : "warn"} sub={<>Headroom health <SampleTag /></>} />
      </PostureGrid>
      ...
      >
        ...
        >
          ...
          <Select label="Limit Type" value={fType} onChange={setFType} options={facet(records.map((r) => r.limitType))} />
        ...
        <DirectoryTable
          ...
          bulkActions={(ids, clear) => (
            <>
              ...
              <HeaderButton icon={<Pencil size={13} />} onClick={clear}>Edit</HeaderButton>
...
const DRAWER_TABS = [
  ...
  { id: "thresholds", label: "Thresholds", icon: <SlidersHorizontal size={13} /> },
        ...
        <Section title="Statistics" sample>
          <KVGrid
            ...
            items={[
              ...
              { k: "Maximum Limit", v: `${rec.limit} ${rec.unit}`, sample: true },
              { k: "Peak Usage", v: `${rec.peak} ${rec.unit}`, sample: true },
              { k: "Average Usage", v: `${rec.average} ${rec.unit}`, sample: true },
              { k: "Remaining Capacity", v: `${Math.max(0, rec.limit - rec.current)} ${rec.unit}`, sample: true },
...
function UsageTab({ rec }: { rec: Limit }) {
  const list = [
    { id: "current", metric: "Current", current: rec.current, limit: rec.limit, remaining: Math.max(0, rec.limit - rec.current), util: (rec.current / rec.limit) * 100 },
    { id: "peak", metric: "Peak", current: rec.peak, limit: rec.limit, remaining: Math.max(0, rec.limit - rec.peak), util: (rec.peak / rec.limit) * 100 },
    { id: "avg", metric: "Average", current: rec.average, limit: rec.limit, remaining: Math.max(0, rec.limit - rec.average), util: (rec.average / rec.limit) * 100 },
  ];
  const cols: Column<(typeof list)[number]>[] = [
    { key: "metric", header: "Metric", render: (r) => r.metric },
    { key: "current", header: "Value", render: (r) => `${r.current} ${rec.unit}` },
  ...
  return (
    <>
      <Section title="Current resource consumption" sample>
        <StatRow label={rec.resource} value={<UsageBar pct={(rec.current / rec.limit) * 100} />} sample />
      </Section>
  ...
  return (
    <>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
        ...
        <HeaderButton variant="danger" icon={<Trash2 size={13} />}>Remove Threshold</HeaderButton>
        <HeaderButton icon={<RefreshCcw size={13} />}>Reset Defaults</HeaderButton>
        <SampleTag />
      </div>
      <Section title="Warning & enforcement thresholds" sample>
        {thresholds.map((t) => (
          <StatRow
            key={t}
            label={`${t}%`}
            value={t >= 100 ? "Block provisioning" : t >= 90 ? "Alert + require approval" : t >= 80 ? "Warn owner" : "Notify"}
  ...
  return (
    <>
      <Section title="Current enforcement" sample>
        <StatRow label="Enforcement Mode" value={<EnforcementPill status={rec.enforcement} />} sample />
        <StatRow label="Automatic Actions" value={rec.enforcement === "enforced" ? "Block over-limit provisioning" : "Alert only"} sample />
      ...
      <Section title="Supported enforcement modes" sample>
        {ENFORCE_MODES.map((m, i) => (
          <StatRow key={m} label={m} value={i === 2 ? "Selected" : "Available"} tone={i === 2 ? "ok" : undefined} sample />
        ))}
      </Section>
    </>
  );
  ...
  return (
    <>
      ...
      <Section title="Resource usage forecast" sample>
        <StatRow label="Growth Rate" value={`+${rec.growthRate}% / month`} sample />
        <StatRow label="Days Remaining" value={ran ? `${rec.daysRemaining}` : "Run forecast"} tone={rec.daysRemaining < 60 ? "warn" : "ok"} sample />
        <StatRow label="Estimated Exhaustion" value={ran ? `~${rec.daysRemaining} days` : "—"} sample />
        <StatRow label="Recommended Capacity" value={ran ? `${Math.round(rec.limit * 1.3)} ${rec.unit}` : "—"} sample />
      </Section>
    </>
  );
}
...
function ActivityTab({ rec }: { rec: Limit }) {
  const events = ["Limit Created", "Limit Updated", "Threshold Triggered", "Limit Exceeded", "Limit Increased", "Forecast Generated"];
  return (
    <>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
...
function AuditTab() {
  const events = ["Limit Created", "Limit Modified", "Threshold Changed", "Limit Exceeded", "Limit Increased", "Forecast Executed"];
  return (
    <>
