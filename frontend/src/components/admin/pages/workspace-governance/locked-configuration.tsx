...
const LOCK_NAMES: { name: string; cat: Category; val: string }[] = [
  ...
  { name: "Cloud Provider Allow-list", cat: "Cloud Resources", val: "AWS, Azure" },
...
export function LockedConfigurationView() {
  ...
  const toolbar: CommandItem[] = [
    ...
    { key: "modify", label: "Modify Lock", icon: <SlidersHorizontal size={15} />, disabled: true },
    ...
    { key: "lock", label: "Lock Configuration", icon: <Lock size={15} />, disabled: true },
    { key: "unlock", label: "Unlock Configuration", icon: <Unlock size={15} />, disabled: true },
    { key: "approve", label: "Approve Unlock", icon: <Check size={15} />, disabled: true },
    { key: "validate", label: "Validate Lock", icon: <ClipboardCheck size={15} />, disabled: true },
    { key: "conflicts", label: "Detect Conflicts", icon: <ShieldCheck size={15} />, disabled: true },
    { key: "impact", label: "Run Impact Analysis", icon: <Network size={15} />, disabled: true },
    { key: "version", label: "Lock History", icon: <History size={15} />, disabled: true },
  ];
  ...
  const cols: Column<LockRec>[] = [
    ...
    { key: "scope", header: "Lock Scope", sortValue: (r) => r.scope, render: (r) => r.scope },
    {
      key: "inherited",
      header: "Inherited",
    ...
    { key: "level", header: "Lock Level", sortValue: (r) => r.level, render: (r) => <LevelBadge level={r.level} /> },
  ...
  return (
    <>
      <PostureGrid>
        <PostureCard title="Active Locks" value={activeLocks} tone="ok" sub={<>Enforced org-wide <SampleTag /></>} />
        <PostureCard title="Locked Workspaces" value={lockedWorkspaces} tone="ok" sub={<>Inheriting a lock <SampleTag /></>} />
        <PostureCard title="Pending Unlock Requests" value={pendingUnlock} tone={pendingUnlock > 0 ? "warn" : "ok"} sub={<>Awaiting review <SampleTag /></>} />
        <PostureCard title="High-Risk Unlocks" value={highRiskUnlock} tone={highRiskUnlock > 0 ? "danger" : "ok"} sub={<>Critical/High risk <SampleTag /></>} />
        <PostureCard title="Locked Policies" value={lockedPolicies} tone="ok" sub={<>Protected by locks <SampleTag /></>} />
        <PostureCard title="Blocked Overrides" value={blockedOverrides} tone={blockedOverrides > 0 ? "warn" : "ok"} sub={<>Prevented deviations <SampleTag /></>} />
        <PostureCard title="Lock Violations" value={0} tone="ok" sub={<>Attempted changes <SampleTag /></>} />
      </PostureGrid>
      ...
      >
        ...
        >
          ...
          <Select label="Lock Scope" value={fScope} onChange={setFScope} options={facet(records.map((r) => r.scope))} />
          <Select label="Lock Level" value={fLevel} onChange={setFLevel} options={facet(records.map((r) => r.level))} />
        ...
        <DirectoryTable
          ...
          bulkActions={(ids, clear) => (
            <>
              <HeaderButton icon={<Lock size={13} />} onClick={clear}>Lock ({ids.length})</HeaderButton>
              <HeaderButton icon={<Unlock size={13} />} onClick={clear}>Unlock</HeaderButton>
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <HeaderButton variant="primary" icon={<Lock size={14} />} onClick={() => navigate("/admin/workspace-governance?tab=inheritance")}>
              Create Lock
            </HeaderButton>
          </>
        }
...
const DRAWER_TABS = [
  ...
  { id: "effective", label: "Effective Configuration", icon: <ShieldCheck size={13} /> },
    ...
    <SideRailDrawer
      ...
      footer={
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end", width: "100%" }}>
          <HeaderButton icon={<Network size={13} />}>Impact Analysis</HeaderButton>
      ...
      {sub === "statistics" && (
        <Section title="Statistics" sample>
          <KVGrid
            cols={3}
            items={[
              { k: "Inherited Workspaces", v: rec.inheritedWorkspaces, sample: true },
  ...
  return (
    <Section title="Locked settings — protected configuration" sample>
      <div style={{ fontSize: 12.5, color: T.textMuted, marginBottom: 10 }}>
        The configuration values protected by this lock, who locked them, and the enforcement level.
      </div>
      <StatRow label={rec.name} value={`${rec.currentValue} · Locked by ${rec.owner}`} tone="ok" sample />
      <StatRow label="Lock Level" value={<LevelBadge level={rec.level} />} sample />
...
function ScopeTab({ rec }: { rec: LockRec }) {
  return (
    <Section title="Lock scope — where the lock applies" sample>
      <div style={{ fontSize: 12.5, color: T.textMuted, marginBottom: 10 }}>
        Hierarchy: Organization → Business Unit → Workspace Template → Workspace.
      </div>
      {SCOPES.map((s) => (
        <StatRow
          key={s}
          label={s}
          value={s === rec.scope ? "Owning scope (locked here)" : rec.inherited ? "Inherited" : "—"}
          tone={s === rec.scope ? "ok" : undefined}
          sample
        />
      ))}
      <StatRow label="Effective Scope" value={`${rec.inheritedWorkspaces} workspaces`} sample />
    </Section>
  );
}
...
function DepsTab({ rec }: { rec: LockRec }) {
  ...
  const list = Array.from({ length: 2 + (n % 4) }, (_, i) => {
    ...
    return {
      id: `${rec.id}-d-${i}`,
      dep: pick(["Encryption Policy", "IAM Baseline", "Network Policy", "Backup Policy", "Compliance Control"], m),
...
function EffectiveTab({ rec }: { rec: LockRec }) {
  return (
    <Section title="Effective configuration — after applying locks" sample>
      <div style={{ fontSize: 12.5, color: T.textMuted, marginBottom: 10 }}>
        A requested override against a locked configuration is rejected unless an unlock is approved.
      </div>
      <KVGrid
        items={[
          { k: "Configuration", v: rec.name },
          { k: "Inherited Value", v: rec.currentValue, sample: true },
          { k: "Requested Override", v: rec.blockedOverrides > 0 ? "Change requested" : "None", sample: true },
          { k: "Effective Value", v: `${rec.currentValue} (Locked)`, sample: true },
...
function UnlockTab({ rec }: { rec: LockRec }) {
  ...
  const list = Array.from({ length: rec.unlockRequests }, (_, i) => {
    ...
    return {
      ...
      reason: pick(["Migration", "Emergency Response", "Customer Requirement", "Regulatory Exception", "Temporary Testing"], m),
  ...
  return (
    <>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
        ...
        <HeaderButton icon={<Inbox size={13} />}>Request More Information</HeaderButton>
        <SampleTag />
      </div>
      <div style={{ fontSize: 12.5, color: T.textMuted, marginBottom: 10 }}>
        Workflow: Request → Security Review → Governance Review → Approval → Unlock.
      </div>
      {list.length ? (
        <DirectoryTable columns={cols} rows={list} />
      ) : (
        <EmptyState icon={<Inbox size={18} />} title="No unlock requests" hint="No requests to remove this lock are pending." />
      )}
    </>
  );
}
