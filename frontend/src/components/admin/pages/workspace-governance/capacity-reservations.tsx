...
const SAMPLE_RESERVATIONS: Reservation[] = RES_SEED.map(
  ({ name, resource, unit }, i) => {
    const id = `CR-${(10000 + i * 37).toString()}`;
    const n = hashId(id + name);
    const status = pick<Status>(
      ["Active", "Active", "Pending", "Scheduled", "Expiring", "Expired"],
      n,
    );
...
export function CapacityReservationsView() {
  ...
  const activeReservations = records.filter(
    (r) => r.status === "Active",
  ).length;
  const reservedCapacity = records.reduce((a, r) => a + r.reserved, 0);
  const availableReserved = records.reduce(
    (a, r) => a + (r.reserved - r.consumed),
    0,
  );
  const upcoming = records.filter((r) => r.status === "Scheduled").length;
  const expiring = records.filter((r) => r.status === "Expiring").length;
  const reservationUtil = Math.round(
    (records.reduce((a, r) => a + r.consumed, 0) / reservedCapacity) * 100,
  );
  const successRate =
    Math.round((activeReservations / records.length) * 100) + 20;
      ...
      <DiscoveryListView
        ...
        pills={[
          ...
          },
          {
            ...
            value: fType,
            onChange: setFType,
            options: facet(records.map((r) => r.reservationType)),
          },
          {
            key: "status",
            label: "Status",
        ...
        presets={[
          { label: "All capacity reservations", onApply: clearFilters },
        ]}
        filterRightSlot={
        ...
        <Section title="Statistics" sample>
          <KVGrid
            ...
            items={[
              {
                k: "Reserved Capacity",
                v: `${rec.reserved} ${rec.unit}`,
                sample: true,
              },
              {
                k: "Allocated Capacity",
                v: `${rec.allocated} ${rec.unit}`,
                sample: true,
              },
              {
                k: "Consumed Capacity",
                v: `${rec.consumed} ${rec.unit}`,
                sample: true,
              },
              {
                k: "Available Capacity",
                v: `${Math.max(0, rec.reserved - rec.consumed)} ${rec.unit}`,
                sample: true,
              },
              {
                k: "Reservation Utilization",
                v: `${Math.round((rec.consumed / rec.reserved) * 100)}%`,
                sample: true,
              },
...
function ResourcesTab({ rec }: { rec: Reservation }) {
  const list = RES_TYPES.slice(0, 5).map((r, i) => {
    ...
    return {
      ...
      reserved,
      allocated: Math.round(reserved * 0.9),
      available: Math.round(reserved * 0.3),
      unit: i === 0 ? rec.unit : pick(["vCPU", "GiB", "TB", "GPU"], m),
    };
  });
    ...
    <Section title="Schedule — reservation timing" sample>
      <KVGrid
        items={[
          ...
          {
            k: "Duration",
            v: `${30 + (hashId(rec.id) % 90)} days`,
            sample: true,
          },
          { k: "Expiration", v: rec.endDate, sample: true },
          {
            k: "Recurring",
            v: hashId(rec.id) % 2 === 0 ? "Annual" : "One-time",
            sample: true,
          },
...
function AllocationTab({ rec }: { rec: Reservation }) {
  ...
  const list = targets.map((t) => {
    ...
    return {
      ...
      target: t,
      reserved,
      allocated: Math.round(reserved * 0.8),
      remaining: Math.round(reserved * 0.2),
    };
  });
  const cols: Column<(typeof list)[number]>[] = [
    { key: "target", header: "Target", render: (r) => r.target },
    {
      key: "reserved",
      header: "Reserved",
      render: (r) => `${r.reserved} ${rec.unit}`,
    },
    {
      key: "allocated",
      header: "Allocated",
      render: (r) => `${r.allocated} ${rec.unit}`,
    },
    {
      key: "remaining",
      header: "Remaining",
...
function UtilizationTab({ rec }: { rec: Reservation }) {
  ...
  return (
    <Section
      title="Reservation utilization — how reserved capacity is used"
      sample
    >
      ...
      <StatRow
        label="Remaining"
        value={`${Math.max(0, rec.reserved - rec.consumed)} ${rec.unit}`}
        sample
      />
      <StatRow
        label="Utilization"
        value={<UsageBar pct={util} tone={util < 40 ? T.warning : T.success} />}
        sample
      />
      <StatRow
        label="Efficiency"
        value={util < 40 ? "Underused — consider releasing" : "Efficient"}
        tone={util < 40 ? "warn" : "ok"}
        sample
      />
    </Section>
  );
    ...
    <>
      ...
      <Section title="Reservation demand forecast" sample>
        <StatRow
          ...
          value={ran ? "+35% next quarter" : "Run forecast"}
          sample
        />
        <StatRow
          label="Capacity Shortage"
          value={ran ? "Predicted in 45 days" : "—"}
          tone={ran ? "warn" : undefined}
          sample
        />
        <StatRow
          label="Recommended Reservations"
          value={ran ? "2 additional pools" : "—"}
          sample
        />
