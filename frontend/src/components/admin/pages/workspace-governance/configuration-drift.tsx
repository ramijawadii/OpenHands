...
const FRAMEWORKS = ["ISO 27001", "SOC 2", "PCI DSS", "HIPAA", "NIST", "CIS", "CSA CCM", "GDPR"];
const DETECTION = ["Continuous Scan", "Scheduled Scan", "Cloud Event", "Policy Evaluation", "Manual Scan"];
const OWNERS = ["Platform Team", "Cloud Team", "Security Team", "Aisha Khan", "Marco Rossi"];
...
const DRIFTS_SEED: { name: string; cat: Category; exp: string; act: string }[] = [
  { name: "Default Region", cat: "Cloud", exp: "eu-west-1", act: "us-east-1" },
  { name: "Encryption at Rest", cat: "Security", exp: "Enabled", act: "Disabled" },
  { name: "MFA Enforcement", cat: "Identity", exp: "Enforced", act: "Optional" },
  { name: "Audit Logging", cat: "Security", exp: "Enabled", act: "Disabled" },
  { name: "Firewall Rule", cat: "Networking", exp: "Default-deny", act: "0.0.0.0/0 allow" },
  { name: "Approved AI Model", cat: "AI Platform", exp: "GPT-5 Enterprise", act: "GPT-4o" },
  { name: "Backup Retention", cat: "Platform", exp: "35 days", act: "7 days" },
  { name: "Compliance Framework", cat: "Compliance", exp: "SOC 2", act: "None" },
  { name: "IAM Role Policy", cat: "Identity", exp: "Least privilege", act: "AdministratorAccess" },
  { name: "Automation Approval", cat: "Automation", exp: "Required", act: "Auto-run" },
  { name: "TLS Version", cat: "Security", exp: "TLS 1.2", act: "TLS 1.0" },
  { name: "Storage Public Access", cat: "Cloud", exp: "Blocked", act: "Public" },
  { name: "Network Policy", cat: "Networking", exp: "Namespaced", act: "Open" },
...
const SAMPLE_DRIFT: Drift[] = DRIFTS_SEED.map(({ name, cat, exp, act }, i) => {
  ...
  const severity = pick<Severity>(["Critical", "High", "High", "Medium", "Low"], n);
  const status = pick<Status>(
    ["Active", "Active", "Pending Remediation", "Approved", "Remediated", "Ignored"],
    n,
  );
  return {
    id,
    ...
    severity,
    status,
    detected: pick(["5 minutes ago", "2 hours ago", "yesterday", "3 days ago"], n),
    workspace: pick(WORKSPACES, n >> 1),
    environment: pick(ENVIRONMENTS, n >> 2),
    cause: pick<Cause>(
      ["Manual Change", "Cloud Console", "Automation", "API", "Infrastructure as Code", "Emergency Change", "Provider Change"],
      n,
    ),
    actor: pick(OWNERS, n),
...
export function ConfigurationDriftView() {
  ...
  const hasFilters = !!(search || fSev || fStatus || fWs || fEnv || fCat || fFramework || fDetection);
  const clearFilters = () => {
    setSearch("");
    setFSev("");
    setFStatus("");
  ...
  const pendingRemediation = records.filter((r) => r.status === "Pending Remediation").length;
  const autoRemediated = records.filter((r) => r.status === "Remediated").length;
  const complianceViolations = records.reduce((a, r) => a + r.complianceViolations, 0);
  const policyViolations = records.reduce((a, r) => a + r.policyViolations, 0);
  const avgAge = Math.round(records.reduce((a, r) => a + r.ageHours, 0) / records.length);

  const toolbar: CommandItem[] = [
    { key: "scan", label: "Scan Workspace", icon: <ScanLine size={15} />, onClick: () => setSelId(null) },
    { key: "scanall", label: "Scan All", icon: <ScanLine size={15} />, disabled: true },
    ...
    { key: "approve", label: "Approve Drift", icon: <Check size={15} />, disabled: true },
    { key: "reject", label: "Reject Drift", icon: <X size={15} />, disabled: true },
    { key: "ignore", label: "Ignore Drift", icon: <EyeOff size={15} />, disabled: true },
    { key: "exception", label: "Create Exception", icon: <FilePlus2 size={15} />, disabled: true },
    { key: "compare", label: "Compare Baseline", icon: <GitCompare size={15} />, disabled: true },
    { key: "auto", label: "Auto Remediate", icon: <Wrench size={15} />, disabled: true },
    { key: "restore", label: "Restore Baseline", icon: <RotateCcw size={15} />, disabled: true },
  ];
  ...
  const cols: Column<Drift>[] = [
    ...
    { key: "diff", header: "Expected → Actual", render: (r) => <Diff expected={r.expected} actual={r.actual} /> },
    { key: "category", header: "Category", sortValue: (r) => r.category, render: (r) => r.category },
    { key: "severity", header: "Severity", sortValue: (r) => SEV_ORDER[r.severity], render: (r) => <SeverityBadge severity={r.severity} /> },
    { key: "status", header: "Status", sortValue: (r) => r.status, render: (r) => <StatusBadge status={r.status} /> },
    { key: "detected", header: "Detected", sortValue: (r) => r.ageHours, render: (r) => r.detected },
  ...
  return (
    <>
      <PostureGrid>
        <PostureCard title="Active Drift" value={activeDrift} tone={activeDrift > 0 ? "danger" : "ok"} sub={<>Unresolved deviations <SampleTag /></>} />
        <PostureCard title="Critical Drift" value={criticalDrift} tone={criticalDrift > 0 ? "danger" : "ok"} sub={<>Highest severity <SampleTag /></>} />
        <PostureCard title="Approved Drift" value={approvedDrift} tone="ok" sub={<>Accepted via governance <SampleTag /></>} />
        <PostureCard title="Pending Remediation" value={pendingRemediation} tone={pendingRemediation > 0 ? "warn" : "ok"} sub={<>Awaiting fix <SampleTag /></>} />
        <PostureCard title="Auto Remediated" value={autoRemediated} tone="ok" sub={<>Restored to baseline <SampleTag /></>} />
        <PostureCard title="Compliance Violations" value={complianceViolations} tone={complianceViolations > 0 ? "warn" : "ok"} sub={<>Framework impact <SampleTag /></>} />
        <PostureCard title="Policy Violations" value={policyViolations} tone={policyViolations > 0 ? "warn" : "ok"} sub={<>Governance impact <SampleTag /></>} />
        <PostureCard title="Average Drift Age" value={`${avgAge}h`} tone={avgAge <= 24 ? "ok" : "warn"} sub={<>Detect to resolve <SampleTag /></>} />
      </PostureGrid>
      ...
      >
        ...
        >
          <Select label="Severity" value={fSev} onChange={setFSev} options={facet(records.map((r) => r.severity))} />
          ...
          <Select label="Detection Method" value={fDetection} onChange={setFDetection} options={facet(records.map((r) => r.detection))} />
        </FilterBar>
        ...
        <DirectoryTable
          ...
          bulkActions={(ids, clear) => (
            <>
              ...
              <HeaderButton icon={<X size={13} />} onClick={clear}>Reject</HeaderButton>
              <HeaderButton icon={<RotateCcw size={13} />} onClick={clear}>Restore Baseline</HeaderButton>
              <HeaderButton icon={<Wrench size={13} />} onClick={clear}>Auto Remediate</HeaderButton>
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <HeaderButton variant="primary" icon={<ScanLine size={14} />} onClick={() => navigate("/admin/workspace-governance?tab=inheritance")}>
              Scan All
            </HeaderButton>
          </>
        }
...
const DRAWER_TABS = [
  ...
  { id: "comparison", label: "Configuration Comparison", icon: <GitCompare size={13} /> },
  { id: "rootcause", label: "Root Cause", icon: <SearchIcon size={13} /> },
  { id: "governance", label: "Governance Impact", icon: <ShieldCheck size={13} /> },
  { id: "compliance", label: "Compliance Impact", icon: <BadgeCheck size={13} /> },
        ...
        <Section title="Statistics" sample>
          <KVGrid
            ...
            items={[
              ...
              { k: "Compliance Violations", v: rec.complianceViolations, sample: true },
              { k: "Affected Resources", v: rec.affectedResources, sample: true },
              { k: "Related Alerts", v: rec.relatedAlerts, sample: true },
              { k: "Remediation Attempts", v: rec.remediationAttempts, sample: true },
            ]}
          />
        </Section>
      )}
...
function ComparisonTab({ rec }: { rec: Drift }) {
  const list = [
    { id: "1", conf: rec.name, expected: rec.expected, actual: rec.actual, source: "Organization Default", changedBy: rec.actor },
  ];
  const cols: Column<(typeof list)[number]>[] = [
    { key: "conf", header: "Configuration", render: (r) => r.conf },
    { key: "expected", header: "Expected", render: (r) => <span style={{ color: T.success }}>{r.expected}</span> },
    { key: "actual", header: "Actual", render: (r) => <span style={{ color: T.danger }}>{r.actual}</span> },
...
function GovernanceTab({ rec }: { rec: Drift }) {
  return (
    <Section title="Governance impact — policy evaluation" sample>
      <StatRow label="Affected Policies" value={rec.policyViolations} tone={rec.policyViolations > 0 ? "warn" : "ok"} sample />
      <StatRow label="Broken Inheritance" value={rec.policyViolations > 0 ? "Yes" : "No"} sample />
      <StatRow label="Override Status" value={rec.status === "Approved" ? "Approved override" : "Unapproved"} sample />
      <StatRow label="Lock Status" value={rec.severity === "Critical" ? "Violates lock" : "No lock"} tone={rec.severity === "Critical" ? "warn" : undefined} sample />
      <StatRow label="Approval Required" value={rec.severity === "High" || rec.severity === "Critical" ? "Yes" : "No"} sample />
      <StatRow label="Severity" value={<SeverityBadge severity={rec.severity} />} sample />
    </Section>
  );
}
...
function ComplianceTab({ rec }: { rec: Drift }) {
  return (
    <Section title="Compliance impact — regulatory evaluation" sample>
      ...
      <StatRow label="Failed Controls" value={rec.complianceViolations} tone={rec.complianceViolations > 0 ? "warn" : "ok"} sample />
      <StatRow label="Evidence Impact" value={rec.complianceViolations > 0 ? "Evidence invalidated" : "None"} sample />
      <StatRow label="Audit Risk" value={rec.severity} sample />
      <StatRow label="Compliance Score" value={`${100 - rec.complianceViolations * 6}%`} tone={rec.complianceViolations > 0 ? "warn" : "ok"} sample />
    </Section>
  );
}

function RemediationTab() {
  const actions = ["Restore Baseline", "Approve Drift", "Ignore", "Create Exception", "Create Automation", "Assign Incident"];
  ...
  return (
    <>
      ...
      <Section title="Remediation modes" sample>
        {modes.map((m, i) => (
          <StatRow key={m} label={m} value={i === 0 ? "Selected" : "Available"} tone={i === 0 ? "ok" : undefined} sample />
        ))}
      </Section>
    </>
  );
...
function ActivityTab({ rec }: { rec: Drift }) {
  const events = ["Drift Detected", "Alert Generated", "Approval Requested", "Baseline Restored", "Exception Created", "Automation Executed"];
  return (
    <>
...
function AuditTab() {
  const events = ["Drift Detected", "Drift Approved", "Drift Rejected", "Baseline Restored", "Exception Approved", "Automation Executed"];
  return (
    <>
