...
type Category = "Workspace" | "Governance" | "Security" | "Compliance" | "Lifecycle" | "Automation" | "Resource" | "Identity" | "Notification";
type Operation = "Create" | "Update" | "Delete" | "Assign" | "Unassign" | "Enable" | "Disable" | "Rollback" | "Clone";

const CATEGORIES: Category[] = ["Workspace", "Governance", "Security", "Compliance", "Lifecycle", "Automation", "Resource", "Identity", "Notification"];
const OPERATIONS: Operation[] = ["Create", "Update", "Delete", "Assign", "Unassign", "Enable", "Disable", "Rollback", "Clone"];
const ADMINS = ["john.smith", "alice.jones", "gov.admin", "sec.admin", "comp.admin"];
...
const POLICY_NAMES = ["Workspace Creation Policy", "Production Security Baseline", "Data Residency — EU", "MFA Enforcement", "SOC 2 Framework", "Retention Policy", "Automation Approval", "RBAC Baseline", "Encryption Standard", "Network Egress Policy", "Naming Convention", "Approval Chain Policy"];
...
const SAMPLE_POLICY_CHANGES: PolicyChange[] = Array.from({ length: 18 }, (_, i) => {
  ...
  return {
    ...
    admin: pick(ADMINS, n),
    result: pick<AuditResult>(["Success", "Success", "Success", "Success", "Warning"], n),
    version: `v${2 + (n % 6)}.${n % 9}`,
    previousVersion: `v${1 + (n % 6)}.${(n + 3) % 9}`,
    businessUnit: pick(BUSINESS_UNITS, n),
    approvalStatus: pick<"Approved" | "Pending" | "Rejected">(["Approved", "Approved", "Approved", "Pending", "Rejected"], n),
...
export function PolicyChangesView() {
  ...
  const rows = records.filter((r) => {
    ...
    return (
      (!q || r.id.toLowerCase().includes(q) || r.policy.toLowerCase().includes(q) || r.admin.toLowerCase().includes(q) || r.workspace.toLowerCase().includes(q)) &&
  ...
  const hasFilters = !!(search || fCat || fWs || fAdmin || fApproval || fBu);
  const clearFilters = () => {
    setSearch("");
    setFCat("");
    setFWs("");
  ...
  const pendingReviews = records.filter((r) => r.approvalStatus === "Pending").length;
  ...
  const toolbar: CommandItem[] = [
    ...
    { key: "compare", label: "Compare Versions", icon: <GitCompare size={15} />, disabled: true },
    ...
    { key: "rollback", label: "Rollback Analysis", icon: <RotateCcw size={15} />, disabled: true },
  ...
  const cols: Column<PolicyChange>[] = [
    ...
    { key: "operation", header: "Operation", sortValue: (r) => r.operation, render: (r) => r.operation },
    { key: "workspace", header: "Workspace", sortValue: (r) => r.workspace, render: (r) => r.workspace },
    { key: "admin", header: "Administrator", sortValue: (r) => r.admin, render: (r) => r.admin },
  ...
  return (
    <>
      <PostureGrid>
        <PostureCard title="Policy Changes" value={total} tone="ok" sub={<>Recorded this period <SampleTag /></>} />
        <PostureCard title="Active Policies" value={activePolicies} tone="ok" sub={<>Distinct policies <SampleTag /></>} />
        <PostureCard title="Policy Assignments" value={assignments} tone="ok" sub={<>Assignment events <SampleTag /></>} />
        <PostureCard title="Policy Overrides" value={overrides} tone={overrides > 0 ? "warn" : "ok"} sub={<>Inherited overrides <SampleTag /></>} />
        <PostureCard title="Policy Exceptions" value={exceptions} tone={exceptions > 0 ? "warn" : "ok"} sub={<>Approved exceptions <SampleTag /></>} />
        <PostureCard title="Policy Rollbacks" value={rollbacks} tone="ok" sub={<>Reverted policies <SampleTag /></>} />
        <PostureCard title="Pending Reviews" value={pendingReviews} tone={pendingReviews > 0 ? "warn" : "ok"} sub={<>Awaiting approval <SampleTag /></>} />
      ...
      >
        ...
        >
          <Select label="Policy Category" value={fCat} onChange={setFCat} options={facet(records.map((r) => r.category))} />
          <Select label="Workspace" value={fWs} onChange={setFWs} options={facet(records.map((r) => r.workspace))} />
          <Select label="Administrator" value={fAdmin} onChange={setFAdmin} options={facet(records.map((r) => r.admin))} />
          <Select label="Approval Status" value={fApproval} onChange={setFApproval} options={facet(records.map((r) => r.approvalStatus))} />
        ...
        <DirectoryTable
          ...
          empty={<EmptyState icon={<FileCheck2 size={20} />} title="No policy changes found." hint="Adjust filters to review policy modifications across the platform." />}
        />
      </Card>
...
function PolicyDrawer({ rec, onClose }: { rec: PolicyChange; onClose: () => void }) {
  ...
  return (
    <SideRailDrawer
      ...
      footer={
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end", width: "100%" }}>
          ...
          <HeaderButton icon={<UserCheck size={13} />}>View Assignments</HeaderButton>
    ...
    >
      ...
      {tab === "evidence" && <EvidenceTab id={rec.id} items={["Policy Snapshot", "Before/After Diff", "Approval Record", "API Request", "API Response", "Digital Signature", "Correlation IDs"]} />}
      ...
      {sub === "statistics" && (
        <AuditSection title="Statistics" sample>
          <KVGrid
            cols={3}
            items={[
              { k: "Affected Workspaces", v: rec.affectedWorkspaces, sample: true },
              ...
              { k: "Compliance Impact", v: rec.category === "Compliance" ? "Direct" : "Indirect", sample: true },
            ]}
          />
        </AuditSection>
      )}
    ...
    <AuditSection title="Policy details" sample>
      <KVGrid
        items={[
          ...
          { k: "Approval Reference", v: `APR-${(4000 + (hashId(rec.id) % 9000)).toString()}`, sample: true },
          { k: "Source", v: "Governance Center", sample: true },
          { k: "Change Request", v: `CR-${(hashId(rec.id) % 90000).toString()}`, sample: true },
        ]}
      />
    </AuditSection>
  );
    ...
    <>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
        <div style={{ border: `1px solid ${T.border}`, borderRadius: 8, overflow: "hidden" }}>
          <div style={{ fontSize: 11.5, color: T.textMuted, padding: "6px 10px", borderBottom: `1px solid ${T.border}`, background: T.badgeBg }}>Previous ({rec.previousVersion})</div>
          <pre style={{ margin: 0, padding: 10, fontSize: 11.5, color: T.textNav, fontFamily: "monospace", whiteSpace: "pre-wrap" }}>{`rules: 12\nenforcement: monitoring\nassignments: 8`}</pre>
        </div>
        <div style={{ border: `1px solid ${T.border}`, borderRadius: 8, overflow: "hidden" }}>
          <div style={{ fontSize: 11.5, color: T.textMuted, padding: "6px 10px", borderBottom: `1px solid ${T.border}`, background: T.badgeBg }}>Current ({rec.version})</div>
          <pre style={{ margin: 0, padding: 10, fontSize: 11.5, color: T.warning, fontFamily: "monospace", whiteSpace: "pre-wrap" }}>{`rules: 14\nenforcement: enforced\nassignments: ${rec.assignments}`}</pre>
        </div>
      </div>
...
function AssignmentsTab({ rec }: { rec: PolicyChange }) {
  const list = Array.from({ length: rec.assignments }, (_, i) => {
    ...
    return { id: `${rec.id}-a-${i}`, target: `${pick(BUSINESS_UNITS, m)} / ${pick(WORKSPACES, m)}`, type: pick(["Organization", "Business Unit", "Workspace", "Workspace Template"], m), assignment: pick(["Inherited", "Direct", "Override", "Exception"], m), status: pick(["Active", "Active", "Pending"], m) };
  });
  ...
  return (
    <>
      <div style={{ fontSize: 12.5, color: T.textMuted, marginBottom: 10, display: "flex", alignItems: "center", gap: 8 }}>
        Where this policy is assigned (inherited / direct / override / exception). <SampleTag />
      </div>
...
function ImpactTab({ rec }: { rec: PolicyChange }) {
  return (
    <AuditSection title="Impact analysis" sample>
      <StatRow label="Affected Workspaces" value={rec.affectedWorkspaces} sample />
      <StatRow label="Affected Resources" value={rec.affectedWorkspaces * 4} sample />
      <StatRow label="Affected Users" value={rec.affectedWorkspaces * 12} sample />
      <StatRow label="Compliance Impact" value={rec.category === "Compliance" ? "Framework affected" : "None"} tone={rec.category === "Compliance" ? "warn" : "ok"} sample />
      <StatRow label="Security Impact" value={rec.category === "Security" ? "Elevated" : "Low"} tone={rec.category === "Security" ? "warn" : "ok"} sample />
      <StatRow label="Operational Impact" value={rec.riskScore > 50 ? "Elevated" : "Low"} sample />
      <StatRow label="Business Impact" value={rec.riskScore > 60 ? "High" : "Moderate"} sample />
    </AuditSection>
  );
}

function ApprovalTab({ rec }: { rec: PolicyChange }) {
  const stages = ["Submitted", "Governance Review", "Security Review", "Compliance Review", "Approved", "Published"];
  const completed = rec.approvalStatus === "Approved" ? stages.length : rec.approvalStatus === "Rejected" ? 2 : 3;
  return (
        ...
        return (
          <div key={s} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0" }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11.5, color: T.textMuted }}>{rejected ? `${pick(ADMINS, hashId(rec.id))} · rejected` : done ? `${pick(ADMINS, hashId(rec.id) + i)} · approved` : "Pending"}</div>
            </div>
          </div>
        );
      })}
...
function ResourcesTab({ rec }: { rec: PolicyChange }) {
  const list = Array.from({ length: 3 + (hashId(rec.id) % 4) }, (_, i) => {
    ...
    return { id: `${rec.id}-r-${i}`, resource: pick(["Workspace", "Workspace Template", "Automation", "Cloud Account", "Compliance Framework", "Identity"], m), workspace: pick(WORKSPACES, m), status: pick(["Active", "Active", "Pending"], m) };
  });
  const cols: Column<(typeof list)[number]>[] = [
