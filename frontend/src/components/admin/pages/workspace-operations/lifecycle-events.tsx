...
type Phase = "Request" | "Provisioning" | "Activation" | "Operational" | "Maintenance" | "Suspension" | "Archive" | "Restoration" | "Decommission";
type TransitionType = "Manual" | "Automated" | "Scheduled" | "Approval Driven" | "Policy Driven" | "Recovery" | "Rollback";

const PHASES: Phase[] = ["Request", "Provisioning", "Activation", "Operational", "Maintenance", "Suspension", "Archive", "Restoration", "Decommission"];
const TRANSITION_TYPES: TransitionType[] = ["Manual", "Automated", "Scheduled", "Approval Driven", "Policy Driven", "Recovery", "Rollback"];
const WORKSPACES = ["Payments Production", "Retail Web", "Data Lake", "Identity", "Analytics", "Mobile API"];
const TEMPLATES = ["Regulated Production", "Standard", "Sandbox", "Data Platform"];
const BUSINESS_UNITS = ["Finance", "Engineering", "Operations", "Retail", "Corporate"];
const PROVIDERS = ["AWS", "Azure", "GCP", "Kubernetes"];
const ACTORS = ["Automation Engine", "Provisioning Engine", "john.smith", "gov.admin", "Scheduler"];
const EVENTS_BY_PHASE: Record<Phase, string[]> = {
  Request: ["Workspace Requested", "Request Updated", "Request Approved", "Request Cancelled"],
  Provisioning: ["Provisioning Started", "Provisioning Completed", "Provisioning Failed", "Provisioning Retried"],
  Activation: ["Validation Completed", "Workspace Activated", "Activation Failed"],
  Operational: ["Workspace Updated", "Workspace Expanded", "Configuration Applied", "Ownership Changed"],
  Maintenance: ["Maintenance Started", "Maintenance Completed", "Maintenance Extended"],
  Suspension: ["Workspace Suspended", "Suspension Removed", "Emergency Suspension"],
  Archive: ["Archive Started", "Archive Completed", "Archive Failed"],
  Restoration: ["Restore Started", "Restore Completed", "Restore Failed"],
  Decommission: ["Decommission Requested", "Cleanup Completed", "Workspace Deleted"],
};
...
const SAMPLE_EVENTS: LEvent[] = Array.from({ length: 18 }, (_, i) => {
  ...
  return {
    ...
    event,
    actor: pick(ACTORS, n),
    result: failed ? "Failed" : pick<AuditResult>(["Success", "Success", "Success", "Warning"], n),
    durationMin: 1 + (n % 40),
    ...
    severity: failed ? pick<Severity>(["Critical", "High"], n) : pick<Severity>(["Medium", "Low", "Low"], n),
    integrity: pick<Integrity>(["Verified", "Verified", "Verified", "Warning"], n),
    transitionType: pick(TRANSITION_TYPES, n >> 1),
    previousState: pick(["Requested", "Provisioning", "Active", "Maintenance", "Suspended"], n),
    currentState: pick(["Provisioning", "Active", "Maintenance", "Suspended", "Archived"], n + 1),
...
export function LifecycleEventsView() {
  ...
  const rows = records.filter((r) => {
    ...
    return (
      (!q || r.id.toLowerCase().includes(q) || r.workspace.toLowerCase().includes(q) || r.event.toLowerCase().includes(q) || r.actor.toLowerCase().includes(q)) &&
      (!fWs || r.workspace === fWs) &&
  ...
  const hasFilters = !!(search || fWs || fPhase || fTemplate || fResult || fEnv || fBu);
  const clearFilters = () => {
    setSearch("");
    setFWs("");
    setFPhase("");
  ...
  const exceptions = records.filter((r) => r.transitionType === "Policy Driven").length;
  const avgTransition = Math.round(records.reduce((a, r) => a + r.durationMin, 0) / records.length);
  ...
  const cols: Column<LEvent>[] = [
    { key: "timestamp", header: "Timestamp", sortValue: (r) => r.timestamp, render: (r) => <span style={{ fontVariantNumeric: "tabular-nums", fontSize: 12 }}>{r.timestamp}</span> },
    { key: "workspace", header: "Workspace", sortValue: (r) => r.workspace, render: (r) => r.workspace },
    {
      ...
      sortValue: (r) => r.phase,
      render: (r) => (
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6, color: PHASE_TONE[r.phase] }}>
          <span style={{ width: 7, height: 7, borderRadius: "50%", background: PHASE_TONE[r.phase] }} />
          {r.phase}
        </span>
      ),
    },
    { key: "event", header: "Event", sortValue: (r) => r.event, render: (r) => r.event },
    { key: "actor", header: "Actor", sortValue: (r) => r.actor, render: (r) => r.actor },
    { key: "result", header: "Result", sortValue: (r) => r.result, render: (r) => <ResultBadge result={r.result} /> },
  ...
  return (
    <>
      <PostureGrid>
        <PostureCard title="Lifecycle Events" value={total} tone="ok" sub={<>Recorded this period <SampleTag /></>} />
        <PostureCard title="Workspace Requests" value={requests} tone="ok" sub={<>New requests <SampleTag /></>} />
        <PostureCard title="Provisioning Events" value={provisioning} tone="ok" sub={<>Provisioning activity <SampleTag /></>} />
        <PostureCard title="Lifecycle Transitions" value={transitions} tone="ok" sub={<>Successful transitions <SampleTag /></>} />
        <PostureCard title="Failed Transitions" value={failed} tone={failed > 0 ? "danger" : "ok"} sub={<>Unsuccessful <SampleTag /></>} />
        <PostureCard title="Policy Exceptions" value={exceptions} tone={exceptions > 0 ? "warn" : "ok"} sub={<>Policy-driven <SampleTag /></>} />
        <PostureCard title="Avg Transition Time" value={`${avgTransition}m`} tone="ok" sub={<>Per transition <SampleTag /></>} />
        <PostureCard title="Audit Integrity" value={integrityOk ? "Verified" : "Review"} tone={integrityOk ? "ok" : "warn"} sub={<>Hash chain status <SampleTag /></>} />
      </PostureGrid>
      ...
      >
        ...
        >
          ...
          <Select label="Lifecycle Phase" value={fPhase} onChange={setFPhase} options={facet(records.map((r) => r.phase))} />
          <Select label="Workspace Template" value={fTemplate} onChange={setFTemplate} options={facet(records.map((r) => r.template))} />
          <Select label="Result" value={fResult} onChange={setFResult} options={facet(records.map((r) => r.result))} />
        ...
        <DirectoryTable
          ...
          empty={<EmptyState icon={<Repeat size={20} />} title="No lifecycle events found." hint="Adjust filters to review workspace lifecycle transitions." />}
        />
      </Card>
...
const DRAWER_TABS = [
  ...
  { id: "details", label: "Event Details", icon: <SlidersHorizontal size={13} /> },
...
function EventDrawer({ rec, onClose }: { rec: LEvent; onClose: () => void }) {
  ...
  return (
    <SideRailDrawer
      ...
      footer={
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end", width: "100%" }}>
          <HeaderButton icon={<History size={13} />}>View Timeline</HeaderButton>
    ...
    >
      ...
      {tab === "evidence" && <EvidenceTab id={rec.id} items={["Lifecycle Record", "Workflow History", "Automation Logs", "Provisioning Logs", "Approval Records", "API Requests", "Digital Signature", "Correlation IDs"]} />}
      ...
      {sub === "statistics" && (
        <AuditSection title="Statistics" sample>
          <KVGrid
            cols={3}
            items={[
              { k: "Transition Duration", v: `${rec.durationMin} min`, sample: true },
              { k: "Dependent Resources", v: rec.dependentResources, sample: true },
    ...
    <AuditSection title="Event details" sample>
      <KVGrid
        items={[
          ...
          { k: "Execution Method", v: rec.actor.includes("Engine") ? "Automated" : "Manual", sample: true },
...
function TimelineTab({ rec }: { rec: LEvent }) {
  const seq = ["Workspace Requested", "Approval Granted", "Provisioning Started", "Provisioning Completed", "Validation Passed", "Workspace Activated", "Maintenance", "Suspended", "Archived"];
  const currentIdx = seq.findIndex((s) => rec.event.includes(s.split(" ")[0])) >= 0 ? seq.findIndex((s) => rec.event.includes(s.split(" ")[0])) : 3;
  return (
    <AuditSection title="Lifecycle sequence" sample>
      {seq.map((s, i) => (
        <div key={s} style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 0" }}>
          <span style={{ width: 4, height: 24, background: i <= currentIdx ? T.accent : T.border, borderRadius: 2 }} />
          <div style={{ fontSize: 13, color: i === currentIdx ? T.textPrimary : T.textMuted, fontWeight: i === currentIdx ? 600 : 400 }}>{s}</div>
          {i === currentIdx && <span style={{ marginLeft: "auto", fontSize: 11, color: T.accent, border: `1px solid ${T.accent}55`, borderRadius: 99, padding: "2px 9px" }}>this event</span>}
        </div>
      ))}
    </AuditSection>
  );
...
function ResourcesTab({ rec }: { rec: LEvent }) {
  const list = Array.from({ length: rec.dependentResources }, (_, i) => {
    ...
    return { id: `${rec.id}-r-${i}`, resource: pick(["Provisioning Job", "Automation Workflow", "Approval Request", "Cloud Account", "Cluster", "Policy"], m), workspace: rec.workspace, status: pick(["Completed", "Completed", "Pending"], m) };
  });
...
function DepsTab({ rec }: { rec: LEvent }) {
  const deps = ["Required Approvals", "Provisioning Jobs", "Automation Tasks", "Shared Resources", "Parent Workspace", "External Systems"];
  const list = deps.map((d) => {
    const m = hashId(rec.id + d);
    return { id: d, dependency: d, status: pick(["Satisfied", "Satisfied", "Pending", "Blocked"], m) };
  });
  const cols: Column<(typeof list)[number]>[] = [
    { key: "dependency", header: "Dependency", render: (r) => r.dependency },
    { key: "status", header: "Status", render: (r) => <span style={{ color: r.status === "Satisfied" ? T.success : r.status === "Blocked" ? T.danger : T.warning }}>{r.status}</span> },
  ];
  return (
    <>
      <div style={{ fontSize: 12.5, color: T.textMuted, marginBottom: 10, display: "flex", alignItems: "center", gap: 8 }}>
...
function PolicyTab() {
  const policies = ["Lifecycle Policy", "Provisioning Policy", "Retention Policy", "Archive Policy", "Deletion Policy", "Compliance Policy", "Approval Policy"];
  const list = policies.map((p) => {
    const m = hashId(p + "lce");
    return { id: p, policy: p, result: pick(["Passed", "Passed", "Warning", "Exception Applied"], m) };
  });
  const cols: Column<(typeof list)[number]>[] = [
    { key: "policy", header: "Policy", render: (r) => r.policy },
    { key: "result", header: "Result", render: (r) => <span style={{ color: r.result === "Passed" ? T.success : T.warning }}>{r.result}</span> },
  ];
  return (
    <>
      <div style={{ fontSize: 12.5, color: T.textMuted, marginBottom: 10, display: "flex", alignItems: "center", gap: 8 }}>
