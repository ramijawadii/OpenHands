...
const SAMPLE_PACKAGES: EvPackage[] = Array.from({ length: 16 }, (_, i) => {
  ...
  const status = pick<ExportStatus>(
    [
      "Completed",
      "Completed",
      "Completed",
...
export function ExportEvidenceView() {
  ...
  const cols: Column<EvPackage>[] = [
    ...
    },
    {
      ...
      render: (r) => <IntegrityBadge integrity={r.integrity} />,
    },
  ];
  ...
  return (
    <>
      <StatStripPlain
        items={[
          { label: "Evidence Packages", value: packages, tone: "ok" },
          { label: "Exports Generated", value: generated, tone: "ok" },
          { label: "Scheduled Exports", value: scheduled, tone: "ok" },
          {
          ...
          { label: "Download Requests", value: downloads, tone: "ok" },
          {
            label: "Integrity Verified",
            value: `${verified}/${packages}`,
            tone: verified === packages ? "ok" : "warn",
          },
          {
            label: "Storage Used",
            value: `${(storageMb / 1024).toFixed(1)} GB`,
            tone: "ok",
          },
        ]}
      />
      ...
      <DiscoveryListView
        ...
        pills={[
          ...
          },
          {
            ...
            value: fFormat,
            onChange: setFFormat,
            options: facet(records.map((r) => r.format)),
          },
          {
            key: "businessUnit",
            label: "Business Unit",
        ...
        presets={[{ label: "All export & evidence", onApply: clearFilters }]}
        filterRightSlot={
  ...
  return (
    <AuditSection title="Package contents" sample>
      <div style={{ fontSize: 12.5, color: T.textMuted, marginBottom: 8 }}>
        Every artifact is hash-listed in the evidence manifest and covered by
  ...
  return (
    <AuditSection title="Integrity verification" sample>
      ...
      <StatRow
        ...
        value="SHA-256 manifest match"
        tone="ok"
        sample
      />
      <StatRow
        label="Evidence Chain"
        value="Complete chain of custody"
        tone="ok"
        sample
      />
      <StatRow
      ...
      <StatRow
        ...
        tone={rec.integrity === "Verified" ? "ok" : "warn"}
        sample
      />
      <StatRow
        label="Verification Status"
        value={<IntegrityBadge integrity={rec.integrity} />}
        sample
      />
    </AuditSection>
  );
...
function InvestigationsTab({ rec }: { rec: EvPackage }) {
  const list = Array.from({ length: 2 + (hashId(rec.id) % 3) }, (_, i) => {
    ...
    return {
      ...
      ref: `CASE-${(1000 + (m % 9000)).toString()}`,
      status: pick(["Open", "Closed", "In Review"], m),
    };
  });
...
function DownloadsTab({ rec }: { rec: EvPackage }) {
  const list = Array.from({ length: rec.downloadCount }, (_, i) => {
    ...
    return {
      id: `${rec.id}-d-${i}`,
      downloadedBy: pick(REQUESTERS, m),
      timestamp: `2026-07-${(12 + (m % 10)).toString().padStart(2, "0")} ${(9 + (m % 8)).toString().padStart(2, "0")}:${(m % 60).toString().padStart(2, "0")}`,
      ip: `10.0.${m % 250}.${(m >> 3) % 250}`,
      method: pick(["Secure Link", "Console", "API"], m),
  ...
  const cols: Column<(typeof list)[number]>[] = [
    ...
    { key: "method", header: "Method", render: (r) => r.method },
    {
      key: "verification",
      header: "Verification",
      render: (r) => <span style={{ color: T.success }}>{r.verification}</span>,
    },
  ];
  return (
    <>
      ...
      {list.length ? (
        ...
        <EmptyState
          ...
          hint="This package has not been downloaded."
        />
      )}
    </>
  );
    ...
    <>
      ...
      <AuditSection title="Retention" sample>
        ...
        <StatRow
          ...
          value="Immutable object store (WORM)"
          sample
        />
        <StatRow
          label="Legal Hold"
          value={rec.legalHold ? "Active" : "None"}
          tone={rec.legalHold ? "warn" : undefined}
          sample
        />
        <StatRow
          label="Deletion Schedule"
          value={rec.legalHold ? "Suspended (hold)" : rec.expires}
          sample
        />
      </AuditSection>
    </>
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: 11.5, color: T.textMuted }}>
              {pick(REQUESTERS, hashId(rec.id) + i)} ·{" "}
