...
const RESULT_TONE: Record<AuditResult, string> = { Success: T.success, Failed: T.danger, Warning: T.warning };
const SEVERITY_TONE: Record<Severity, string> = { Critical: T.danger, High: T.warning, Medium: T.accent, Low: T.textMuted };
export const SEVERITY_ORDER: Record<Severity, number> = { Critical: 0, High: 1, Medium: 2, Low: 3 };
const INTEGRITY_TONE: Record<Integrity, string> = { Verified: T.success, Warning: T.warning, Failed: T.danger };
...
export function AuditSection({ title, children, sample }: { title: string; children: React.ReactNode; sample?: boolean }) {
  return (
...
export function EvidenceTab({ id, items }: { id: string; items?: string[] }) {
  const list = items ?? ["JSON Record", "API Request", "API Response", "Approval Record", "Correlation IDs", "Digital Signature", "Related Logs"];
  return (
    <>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
        ...
        <HeaderButton variant="primary" icon={<FilePlus2 size={13} />}>Generate Evidence Package</HeaderButton>
        <SampleTag />
      </div>
      <AuditSection title="Forensic evidence" sample>
        <div style={{ fontSize: 12.5, color: T.textMuted, marginBottom: 8 }}>
          Cryptographically-signed evidence bundle for record {id}. All artifacts are immutable and hash-chained.
        </div>
        {list.map((it) => (
...
export function AuditVerificationTab({ integrity = "Verified" as Integrity }: { integrity?: Integrity }) {
  return (
    <AuditSection title="Audit verification" sample>
      <StatRow label="Integrity Status" value={<IntegrityBadge integrity={integrity} />} sample />
      <StatRow label="Hash Verification" value="SHA-256 chain valid" tone="ok" sample />
      <StatRow label="Timestamp Validation" value="RFC 3161 TSA verified" tone="ok" sample />
      <StatRow label="Signature Verification" value="Ed25519 signature valid" tone="ok" sample />
      <StatRow label="Chain Validation" value="Linked to previous record" tone="ok" sample />
      <StatRow label="Tamper Detection" value={integrity === "Verified" ? "No tampering detected" : "Anomaly flagged"} tone={integrity === "Verified" ? "ok" : "warn"} sample />
    </AuditSection>
  );
}

// ── Administrative Activity leaf ────────────────────────────────────────────────────────────────────
type Category = "Governance" | "Provisioning" | "Automation" | "Security" | "Compliance" | "Identity" | "Configuration" | "Organization";

const CATEGORIES: Category[] = ["Governance", "Provisioning", "Automation", "Security", "Compliance", "Identity", "Configuration", "Organization"];
const ADMINS = ["john.smith", "alice.jones", "m.rossi", "p.nair", "s.lopez", "gov.admin"];
...
const RESOURCE_TYPES = ["Workspace Policy", "Workspace", "Cloud Account", "Role", "Secret", "Automation Template", "Compliance Assignment", "Provisioning Job"];
const ACTIONS = ["Updated Workspace Policy", "Created Workspace", "Rotated Secret", "Assigned Role", "Executed Automation", "Archived Workspace", "Updated MFA Policy", "Assigned Framework", "Deleted Automation", "Escalated Privilege"];
...
const SAMPLE_ACTIVITY: Activity[] = Array.from({ length: 18 }, (_, i) => {
  ...
  const result = pick<AuditResult>(["Success", "Success", "Success", "Success", "Failed", "Warning"], n);
  ...
  return {
    ...
    result,
    severity: privileged ? pick<Severity>(["Critical", "High", "High", "Medium"], n) : pick<Severity>(["Medium", "Low", "Low"], n),
    ...
    previousValue: pick(["Enabled", "v3", "Read-only", "eu-west-1", "Optional"], n),
...
export function AdministrativeActivityView() {
  ...
  const rows = records.filter((r) => {
    ...
    return (
      (!q || r.id.toLowerCase().includes(q) || r.admin.toLowerCase().includes(q) || r.action.toLowerCase().includes(q) || r.workspace.toLowerCase().includes(q)) &&
  ...
  const hasFilters = !!(search || fAdmin || fWs || fCat || fResult || fSeverity || fBu);
  const clearFilters = () => {
    setSearch("");
    setFAdmin("");
    setFWs("");
  ...
  const configChanges = records.filter((r) => r.category === "Configuration" || r.action.includes("Updated")).length;
  const permChanges = records.filter((r) => r.action.includes("Role") || r.action.includes("Privilege")).length;
  ...
  const toolbar: CommandItem[] = [
    ...
    { key: "case", label: "Create Case", icon: <FilePlus2 size={15} />, disabled: true },
    { key: "retention", label: "Retention Settings", icon: <History size={15} />, disabled: true },
  ];
  ...
  const cols: Column<Activity>[] = [
    ...
    { key: "action", header: "Action", sortValue: (r) => r.action, render: (r) => r.action },
    ...
    { key: "ip", header: "IP Address", sortValue: (r) => r.ip, render: (r) => <span style={{ fontSize: 12, color: T.textMuted }}>{r.ip}</span> },
  ];

  return (
    <>
      <PostureGrid>
        <PostureCard title="Administrative Actions" value={actions} tone="ok" sub={<>Recorded this period <SampleTag /></>} />
        <PostureCard title="Active Administrators" value={activeAdmins} tone="ok" sub={<>Distinct actors <SampleTag /></>} />
        <PostureCard title="Configuration Changes" value={configChanges} tone="ok" sub={<>Config-affecting actions <SampleTag /></>} />
        <PostureCard title="Permission Changes" value={permChanges} tone={permChanges > 0 ? "warn" : "ok"} sub={<>Role/privilege edits <SampleTag /></>} />
        <PostureCard title="Failed Operations" value={failed} tone={failed > 0 ? "warn" : "ok"} sub={<>Unsuccessful actions <SampleTag /></>} />
        <PostureCard title="Privileged Actions" value={privileged} tone={privileged > 0 ? "warn" : "ok"} sub={<>Elevated operations <SampleTag /></>} />
        <PostureCard title="Administrative Sessions" value={sessions} tone="ok" sub={<>Login sessions <SampleTag /></>} />
      ...
      >
        ...
        >
          ...
          <Select label="Action Category" value={fCat} onChange={setFCat} options={facet(records.map((r) => r.category))} />
          <Select label="Result" value={fResult} onChange={setFResult} options={facet(records.map((r) => r.result))} />
          <Select label="Severity" value={fSeverity} onChange={setFSeverity} options={facet(records.map((r) => r.severity))} />
        ...
        <DirectoryTable
          ...
          empty={<EmptyState icon={<UserCog size={20} />} title="No administrative activity found." hint="Adjust filters to review administrative actions across the platform." />}
        />
      </Card>
...
const DRAWER_TABS = [
  ...
  { id: "action", label: "Action Details", icon: <SlidersHorizontal size={13} /> },
  { id: "targets", label: "Target Resources", icon: <Boxes size={13} /> },
  { id: "session", label: "Session Information", icon: <MonitorSmartphone size={13} /> },
...
function ActivityDrawer({ rec, onClose }: { rec: Activity; onClose: () => void }) {
    ...
    <SideRailDrawer
      ...
      footer={
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end", width: "100%" }}>
          ...
          <HeaderButton icon={<MonitorSmartphone size={13} />}>View Session</HeaderButton>
      ...
      {sub === "statistics" && (
        <AuditSection title="Statistics" sample>
          <KVGrid
            cols={3}
            items={[
              { k: "Execution Duration", v: `${rec.durationMs} ms`, sample: true },
              { k: "Affected Resources", v: rec.affectedResources, sample: true },
              { k: "Related Activities", v: rec.relatedActivities, sample: true },
              { k: "Privilege Level", v: rec.privileged ? "Privileged" : "Standard", sample: true },
...
function TargetsTab({ rec }: { rec: Activity }) {
  const list = Array.from({ length: rec.affectedResources }, (_, i) => {
    ...
    return { id: `${rec.id}-t-${i}`, resource: pick(RESOURCE_TYPES, m), workspace: rec.workspace, provider: pick(PROVIDERS, m), status: pick(["Modified", "Modified", "Created"], m) };
  });
...
function RelatedTab({ rec }: { rec: Activity }) {
  const list = Array.from({ length: 2 + rec.relatedActivities }, (_, i) => {
    ...
    return { id: `${rec.id}-r-${i}`, activity: pick(["Approval", "Policy Update", "Automation Execution", "Provisioning", "Configuration Change", "Permission Assignment"], m), admin: pick(ADMINS, m), time: `2026-07-${(10 + (m % 18)).toString().padStart(2, "0")}`, result: pick<AuditResult>(["Success", "Success", "Warning"], m) };
  });
  const cols: Column<(typeof list)[number]>[] = [
    ...
    <AuditSection title="Change history — before / after" sample>
      <KVGrid
        items={[
          ...
          { k: "Change Summary", v: `${rec.action} by ${rec.admin}`, sample: true },
        ]}
      />
    </AuditSection>
  );
