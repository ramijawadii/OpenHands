...
type Category = "Workspace" | "Governance" | "Identity" | "Automation" | "Infrastructure" | "Security" | "Compliance" | "Integration" | "Notification";
type Operation = "Create" | "Update" | "Delete" | "Import" | "Restore" | "Rollback" | "Assign" | "Remove";

const CATEGORIES: Category[] = ["Workspace", "Governance", "Identity", "Automation", "Infrastructure", "Security", "Compliance", "Integration", "Notification"];
const OPERATIONS: Operation[] = ["Create", "Update", "Delete", "Import", "Restore", "Rollback", "Assign", "Remove"];
...
const CONFIGS = ["Workspace Policy", "RBAC Configuration", "Encryption Settings", "Network Policy", "Automation Template", "Compliance Assignment", "Cloud Connector", "Notification Routing", "Storage Configuration", "Firewall Rules"];
...
const SAMPLE_CHANGES: Change[] = Array.from({ length: 18 }, (_, i) => {
  ...
  return {
    ...
    operation,
    result,
    severity: pick<Severity>(["Critical", "High", "Medium", "Medium", "Low"], n),
...
export function ConfigurationChangesView() {
  ...
  const rows = records.filter((r) => {
    ...
    return (
      (!q || r.id.toLowerCase().includes(q) || r.configuration.toLowerCase().includes(q) || r.admin.toLowerCase().includes(q) || r.workspace.toLowerCase().includes(q)) &&
      (!fWs || r.workspace === fWs) &&
  ...
  const hasFilters = !!(search || fWs || fCat || fAdmin || fResult || fSeverity || fEnv);
  const clearFilters = () => {
    setSearch("");
    setFWs("");
    setFCat("");
  ...
  const today = records.filter((r) => r.timestamp.includes("2026-07-12")).length + 3;
  ...
  const cols: Column<Change>[] = [
    { key: "timestamp", header: "Timestamp", sortValue: (r) => r.timestamp, render: (r) => <span style={{ fontVariantNumeric: "tabular-nums", fontSize: 12 }}>{r.timestamp}</span> },
    {
      ...
      render: (r) => (
        <span style={{ color: T.textPrimary, display: "inline-flex", alignItems: "center", gap: 8 }}>
          ...
          {r.configuration}
          {r.drift && <span style={{ fontSize: 10, color: T.warning, border: `1px solid ${T.warning}55`, borderRadius: 99, padding: "1px 6px" }}>drift</span>}
        </span>
      ),
    },
    ...
    { key: "severity", header: "Severity", sortValue: (r) => SEVERITY_ORDER[r.severity], render: (r) => <SeverityBadge severity={r.severity} /> },
  ];

  return (
    <>
      <PostureGrid>
        <PostureCard title="Configuration Changes" value={total} tone="ok" sub={<>Recorded this period <SampleTag /></>} />
        <PostureCard title="Today's Changes" value={today} tone="ok" sub={<>Last 24 hours <SampleTag /></>} />
        <PostureCard title="Critical Changes" value={critical} tone={critical > 0 ? "warn" : "ok"} sub={<>High-severity edits <SampleTag /></>} />
        <PostureCard title="Pending Reviews" value={pendingReviews} tone={pendingReviews > 0 ? "warn" : "ok"} sub={<>Awaiting sign-off <SampleTag /></>} />
        <PostureCard title="Configuration Drift" value={drift} tone={drift > 0 ? "warn" : "ok"} sub={<>Off baseline <SampleTag /></>} />
        <PostureCard title="Rollback Events" value={rollbacks} tone="ok" sub={<>Reverted changes <SampleTag /></>} />
        <PostureCard title="Verified Changes" value={verified} tone="ok" sub={<>Integrity-verified <SampleTag /></>} />
      ...
      >
        ...
        >
          ...
          <Select label="Configuration Category" value={fCat} onChange={setFCat} options={facet(records.map((r) => r.category))} />
        ...
        <DirectoryTable
          ...
          empty={<EmptyState icon={<FileDiff size={20} />} title="No configuration changes found." hint="Adjust filters to review configuration modifications across the platform." />}
        />
      </Card>
...
const DRAWER_TABS = [
  ...
  { id: "details", label: "Change Details", icon: <SlidersHorizontal size={13} /> },
...
function ChangeDrawer({ rec, onClose }: { rec: Change; onClose: () => void }) {
  ...
  return (
    <SideRailDrawer
      ...
      footer={
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end", width: "100%" }}>
          ...
          <HeaderButton icon={<RotateCcw size={13} />}>Rollback Analysis</HeaderButton>
    ...
    >
      ...
      {tab === "evidence" && <EvidenceTab id={rec.id} items={["Configuration Snapshot", "Before/After Diff", "API Request", "API Response", "Approval Record", "Correlation IDs", "Digital Signature"]} />}
        ...
        <AuditSection title="Statistics" sample>
          <KVGrid
            ...
            items={[
              ...
              { k: "Changed Properties", v: rec.changedProperties, sample: true },
              { k: "Related Changes", v: rec.relatedChanges, sample: true },
              { k: "Risk Score", v: `${rec.riskScore}/100`, sample: true },
              { k: "Rollback Availability", v: rec.rollbackAvailable ? "Available" : "Unavailable", sample: true },
            ]}
          />
        </AuditSection>
      )}
  ...
  return (
    <>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
        <Select label="Format" value={fmt} onChange={setFmt} options={["JSON", "YAML", "XML", "Table", "Text Diff"].map((f) => ({ value: f, label: f }))} />
        <SampleTag />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div style={{ border: `1px solid ${T.border}`, borderRadius: 8, overflow: "hidden" }}>
          ...
          <pre style={{ margin: 0, padding: 10, fontSize: 11.5, color: T.textNav, fontFamily: "monospace", whiteSpace: "pre-wrap" }}>{`{\n  "${rec.configuration.toLowerCase().replace(/ /g, "_")}": "${rec.before}"\n}`}</pre>
        </div>
        <div style={{ border: `1px solid ${T.border}`, borderRadius: 8, overflow: "hidden" }}>
          ...
          <pre style={{ margin: 0, padding: 10, fontSize: 11.5, color: T.warning, fontFamily: "monospace", whiteSpace: "pre-wrap" }}>{`{\n  "${rec.configuration.toLowerCase().replace(/ /g, "_")}": "${rec.after}"\n}`}</pre>
        </div>
      </div>
...
function ResourcesTab({ rec }: { rec: Change }) {
  const list = Array.from({ length: rec.affectedResources }, (_, i) => {
    ...
    return { id: `${rec.id}-r-${i}`, resource: pick(["Workspace", "Policy", "Cloud Account", "Cluster", "Secret", "Integration"], m), workspace: rec.workspace, provider: pick(PROVIDERS, m), status: pick(["Affected", "Affected", "Pending"], m) };
  });
...
function ImpactTab({ rec }: { rec: Change }) {
  return (
    <AuditSection title="Change impact" sample>
      <StatRow label="Affected Workspaces" value={1 + (hashId(rec.id) % 6)} sample />
      ...
      <StatRow label="Policy Impact" value={rec.category === "Governance" ? "Direct" : "Indirect"} sample />
      <StatRow label="Compliance Impact" value={rec.severity === "Critical" ? "Framework at risk" : "None"} tone={rec.severity === "Critical" ? "warn" : "ok"} sample />
      <StatRow label="Operational Risk" value={rec.severity} sample />
      <StatRow label="Business Impact" value={rec.riskScore > 50 ? "Elevated" : "Low"} tone={rec.riskScore > 50 ? "warn" : "ok"} sample />
    </AuditSection>
  );
}
...
function VersionHistoryTab({ rec }: { rec: Change }) {
  const list = Array.from({ length: 1 + (hashId(rec.id) % 6) }, (_, i) => {
    ...
    return { id: `${rec.id}-v-${i}`, version: `v${8 - i}.${m % 9}`, created: `2026-0${1 + (m % 8)}-${(1 + (m % 27)).toString().padStart(2, "0")}`, admin: pick(ADMINS, m), summary: pick(["Initial", "Policy tightened", "Region change", "Rollback", "Encryption upgrade"], m), status: i === 0 ? "Current" : "Superseded" };
  });
  const cols: Column<(typeof list)[number]>[] = [
    ...
    { key: "status", header: "Status", render: (r) => <span style={{ color: r.status === "Current" ? T.success : T.textMuted }}>{r.status}</span> },
  ];
  return (
    <>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
        <HeaderButton icon={<GitCompare size={13} />}>Compare</HeaderButton>
        <HeaderButton icon={<Waypoints size={13} />}>Rollback Analysis</HeaderButton>
        <SampleTag />
      </div>
      <DirectoryTable columns={cols} rows={list} />
    </>
