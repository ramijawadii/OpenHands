...
const METRICS = ["API calls", "Tokens", "Sandbox compute"];
...
function CustomTooltip({
  ...
  return (
    ...
    >
      <div style={{ color: "var(--cg-text-muted)", marginBottom: 6 }}>
        {label}
      </div>
      {payload.map((p) => (
        <div key={p.name} style={{ color: p.color, marginBottom: 2 }}>
          {p.name}:{" "}
          <span
            style={{ color: "var(--cg-text-primary)", fontFamily: "monospace" }}
          >
...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 12, color: up ? S.success : S.danger }}>
        {delta}
      </div>
    </div>
  );
...
function UsageChart({ range }: { range: string }) {
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div style={{ fontSize: 14, fontWeight: 500, color: S.textPrimary }}>
            Activity Overview
          </div>
          <div style={{ fontSize: 12, color: S.textMuted, marginTop: 2 }}>
            API calls · scans · findings
          </div>
        </div>
      </div>
      <ResponsiveContainer width="100%" height={180}>
        ...
        >
          ...
          <CartesianGrid
            strokeDasharray="3 3"
            stroke="rgba(222,220,206,0.06)"
            vertical={false}
          />
          <XAxis
            dataKey="day"
            tick={{ fill: "#968f85", fontSize: 11 }}
            axisLine={false}
            tickLine={false}
          />
          <YAxis
          ...
          <Tooltip
            ...
            cursor={{ stroke: "rgba(222,220,206,0.12)", strokeWidth: 1 }}
          />
          <Legend
            wrapperStyle={{ fontSize: 11, color: "#968f85", paddingTop: 12 }}
          />
          <Area
            type="monotone"
            dataKey="calls"
...
export default function UsageSettings() {
  ...
  return (
    <div style={{ padding: "40px 48px", maxWidth: 800 }}>
      ...
      {/* Filter bar — workspace · service account · metric (api/token/sandbox compute) · date(above) */}
      <div
        style={{
          display: "flex",
          gap: 10,
      ...
      >
        ...
        <FilterSelect
          value={svcAccount}
          onChange={setSvcAccount}
          all="All service accounts"
          ...
          {METRICS.map((m, i) => (
            <button
              ...
              style={{
                height: 32,
                padding: "0 12px",
                background: metric === m ? S.inputBg : "transparent",
                border: "none",
                borderRight:
                  i !== METRICS.length - 1 ? `1px solid ${S.border}` : "none",
                color: metric === m ? S.textPrimary : S.textMuted,
                fontSize: 13,
                cursor: "pointer",
              }}
            >
      ...
      <div style={{ fontSize: 12, color: S.textMuted, marginBottom: 20 }}>
        Showing <span style={{ color: S.textSecondary }}>{metric}</span> ·{" "}
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        >
          ...
          >
            {["", "Connector", "Scans", "Assets", "Findings", "API Calls"].map(
              (h, i) => (
                <span
                  key={i}
                  style={{
      ...
      <div>
        ...
        >
          ...
          >
            {["", "Member", "Role", "Scans", "Reviewed", "Tokens"].map(
              (h, i) => (
                <span
                  key={i}
                  style={{
