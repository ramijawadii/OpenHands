...
const SAMPLE_SECRETS: Secret[] = [
  {
    ...
    description: "CloudGuard audit role access key for AWS Production",
    createdAt: "2026-03-01",
    updatedAt: "2026-05-12",
    expiresAt: "2026-09-01",
    rotationDays: 90,
    usedBy: ["CloudGuard Scanner", "CLI"],
    tags: ["aws", "iam"],
    version: 3,
  },
  {
    ...
    rotationDays: 180,
    usedBy: ["CloudGuard Agent", "KG Sync"],
    tags: ["gcp", "vertex-ai"],
    version: 2,
  },
  {
  ...
  {
    id: "s7",
    name: "TLS_CERT_CLOUDGUARD_IO",
    type: "certificate",
    env: "production",
    value: "-----BEGIN CERTIFICATE----- MIIDXTCCAkWgAwIBAgIJA••••",
    description:
      "TLS certificate for cloudguard.io — auto-renewed via Let's Encrypt",
    createdAt: "2026-05-01",
    updatedAt: "2026-05-01",
    expiresAt: "2026-08-01",
    usedBy: ["Nginx / Ingress"],
    tags: ["tls", "cert"],
    version: 4,
  },
];
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <button
        ...
        style={{
          background: "none",
          border: "none",
          color: copied ? S.success : S.textMuted,
          cursor: "pointer",
          padding: "2px 4px",
          display: "flex",
          alignItems: "center",
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span style={{ fontSize: 15, fontWeight: 500, color: S.textPrimary }}>
            Add Secret
          </span>
          <button
            type="button"
        ...
        {[
          ...
          },
          {
            ...
            node: (
              <select
                ...
                style={{ ...sel(""), background: S.inputBg }}
              >
                {(
                  Object.entries(TYPE_META) as [
                    SecretType,
                    (typeof TYPE_META)[SecretType],
                  ][]
                ).map(([k, v]) => (
                  <option
                    key={k}
          ...
          },
          {
            ...
            node: (
              ...
              >
                {(
                  Object.entries(ENV_META) as [
                    SecretEnv,
                    (typeof ENV_META)[SecretEnv],
                  ][]
                ).map(([k, v]) => (
                  <option
                    key={k}
          ...
          },
          {
            ...
            node: (
              <input
                ...
                placeholder="aws, iam, production"
                style={{
                  ...sel(""),
                  width: "100%",
                  boxSizing: "border-box" as const,
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
          ...
          <button
            ...
            onClick={() => {
              onSave({
                ...
                tags: tags
                  ...
                  .map((t) => t.trim())
                  .filter(Boolean),
                rotationDays: rotation ? parseInt(rotation) : undefined,
              });
              onClose();
            }}
            disabled={!name || !value}
            style={{
              ...
              padding: "0 16px",
              borderRadius: 6,
              background: !name || !value ? "rgba(45,134,212,0.4)" : S.accent,
              color: "#fff",
              fontSize: 13,
              fontWeight: 500,
              border: "none",
              cursor: !name || !value ? "not-allowed" : "pointer",
            }}
          >
            Add Secret
          </button>
...
function loadSecrets(): Secret[] {
  try {
    const raw = localStorage.getItem(LS_SECRETS);
    if (raw) return JSON.parse(raw) as Secret[];
  } catch {
    /* ignore */
  }
...
export default function VaultSettings() {
  const [secrets, setSecrets] = React.useState<Secret[]>(loadSecrets);
  const [audit, setAudit] = React.useState(AUDIT_LOG);
  ...
  React.useEffect(() => {
    localStorage.setItem(LS_SECRETS, JSON.stringify(secrets));
  }, [secrets]);

  const logAudit = (action: string, secret: string) =>
    setAudit((p) => [
      {
  ...
  const handleAdd = (partial: Partial<Secret>) => {
    ...
    setSecrets((prev) => [
      ...prev,
      {
        ...
        tags: partial.tags ?? [],
        version: 1,
        rotationDays: partial.rotationDays,
      },
    ]);
    logAudit("Created", partial.name ?? "");
  };

  const handleDelete = (id: string) => {
    const name = secrets.find((s) => s.id === id)?.name ?? "";
    setSecrets((prev) => prev.filter((s) => s.id !== id));
    logAudit("Deleted", name);
  };
  ...
  const handleRotate = (id: string) => {
    ...
    logAudit("Rotated", name);
  };

  const expiringCount = secrets.filter((s) => {
    if (!s.expiresAt) return false;
    return (
      Math.ceil((new Date(s.expiresAt).getTime() - Date.now()) / 86400000) < 30
    );
  }).length;
  ...
  return (
    <div style={{ padding: "40px 48px", maxWidth: 880 }}>
      {showAdd && (
        <AddSecretModal onClose={() => setShowAdd(false)} onSave={handleAdd} />
      )}
      ...
      >
        <div>
          ...
          >
            Enterprise-grade secret management — API keys, credentials,
            certificates, tokens.
          </p>
        </div>
        <button
      ...
      >
        {[
          ...
          },
          {
            label: "Expiring <30d",
            value: expiringCount,
            color: expiringCount > 0 ? S.warning : S.textMuted,
          },
          {
            label: "Avg version",
            value: (
              secrets.reduce((a, s) => a + s.version, 0) / secrets.length
            ).toFixed(1),
            color: S.textMuted,
          },
      ...
      >
        ...
        <select
          value={filterEnv}
          onChange={(e) => setFilterEnv(e.target.value as SecretEnv | "all")}
          style={selStyle}
        >
        ...
        <span style={{ marginLeft: "auto", fontSize: 12, color: S.textMuted }}>
      ...
      >
        <div
          style={{
            display: "grid",
            gridTemplateColumns: "28px 2fr 1fr 80px 72px 80px 96px",
            padding: "8px 16px",
              ...
              >
                ...
                >
                  {s.rotationDays && (
                    <button
                      type="button"
                      title={`Rotate (every ${s.rotationDays}d)`}
      ...
      <div>
        ...
        >
          <ChevronDown
            size={14}
            style={{
              transform: showAudit ? "rotate(180deg)" : "rotate(0deg)",
              transition: "transform 150ms ease",
            }}
          />
          Access Audit Log
          ...
          >
            ...
            {audit.map((entry, i) => (
              ...
              >
                ...
                <span
                  style={{ fontSize: 12, color: S.textSecondary, minWidth: 0 }}
                >
                  {entry.actor}
                </span>
                <span
